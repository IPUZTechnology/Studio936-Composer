// Studio 936 Composer - Chart View v2.1.7 Cambio 18 (MAIN CONTROL + ESCENARIO CHART)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.1.7-cambio-18";
  const STYLE_ID = "s936-chart-v217-cambio18";
...
/* Cambio 18 · escenario Chart más alto y controlado por Main */
...
.s936-repeat-practice-cell.active{
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 18 · Main controla instrumento + Chart en escenario alto";
  ...
  function mountInRightPanel({ onChordEdit } = {}) {
    ...
    const statusBar = document.querySelector("section.status-bar") || document.querySelector(".status-bar");
    if (statusBar && !_savedStatusBarState) {
      _savedStatusBarState = {
        el: statusBar,
        display: statusBar.style.display,
        visibility: statusBar.style.visibility,
        height: statusBar.style.height,
        overflow: statusBar.style.overflow
      };
    }
    if (statusBar) {
