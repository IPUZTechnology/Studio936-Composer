// Studio 936 Composer - Suite Pro Structure / ADN Module v4.8.1 Cambio 81
...
// Cambio 81: la altura del panel Ly Letra ya no llena siempre la ventana —
// se mide la altura real del contenido (cabecera + compases) y se usa esa,
// sin pasarse del espacio disponible en pantalla. Evita el hueco vacío
// grande cuando la sección tiene pocos compases.
(function () {
  "use strict";
  ...
  const MODULE_CAMBIO_LABEL = "Cambio 81";

  const STYLE_ID = "s936SuiteProStructureStylesV481Cambio81";
...
`;
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    try {
      ...
      // Cambio 81: antes la altura siempre llenaba la ventana, dejando un
      // hueco vacío grande cuando la sección tiene pocos compases (ej. 12).
      // Ahora se mide la altura real del contenido (cabecera + compases) y
      // se usa esa, sin pasarse nunca del espacio disponible en pantalla.
      const maxHeight = Math.max(300, window.innerHeight - top - 16);
      const contentHeight = head.offsetHeight + body.scrollHeight + 6;
      panel.style.height = Math.min(maxHeight, Math.max(300, contentHeight)) + "px";
    } catch(_) {}
