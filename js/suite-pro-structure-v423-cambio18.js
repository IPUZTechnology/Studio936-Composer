...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      err.innerHTML = "<h4>Cambio número 18 · Error visible en Estructura</h4><p>No se pudo construir el panel. Revisa la primera línea roja de consola.</p>";
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 18 · Main controla Chart + escenario alto");
    root.appendChild(banner);
  }

  function scheduleRightPanelChart(ctx, root) {
    const status = ctx.el("section", "s936-struct-right-status");
    status.textContent = "Cambio número 18 · Estructura abrió. Chart con marcaciones musicales…";
    root.appendChild(status);

    const ensureChartScriptLoaded = () => {
      const src = "js/suite-pro-chart-v216-cambio18.js";
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        status.textContent = "Cambio número 18 · Cargando Chart con marcaciones… intento " + (attempt + 1) + ".";
  ...
  function writeNavigationMarks(marks) {
    ...
    try {
      localStorage.setItem(NAV_KEY, JSON.stringify({
        version: "navigation-v1-cambio18",
