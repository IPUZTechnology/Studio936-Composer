// Studio 936 Composer - Chart View v2.2.0 Cambio 21 (CONSOLA IZQUIERDA + MARCAS EN COMPÁS SIMPLIFICADAS)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.2.0-cambio-21";
  const STYLE_ID = "s936-chart-v220-cambio21";
...
/* Cambio 21 · escenario Chart sin borrar los comandos Groove del Main */
...
/* Cambio 21 · menú contextual profesional por compás */
.s936-ch-bar-head{
  cursor:pointer;
  user-select:none;
}
...
.s936-ch-nav-badge[data-nav-type="loopEnd"]{
  ...
  function showBarContextMenu(event, { sectionKey, barIndex, onRerender }) {
    ...
    menu.className = "s936-bar-menu s936-bar-menu-simple";
    ...
    hint.textContent = "Compás: ritmo local y marcas de partitura/práctica. D.C. y D.S. quedan para sección/canción.";
    ...
    const practiceGrid = group("Práctica");
    ...
    const cleanGrid = group("Limpiar");
    addButton(cleanGrid, "Quitar marcas", () => clearNavigationMarksForBar(sectionKey, barIndex), "danger");

    document.body.append(overlay, menu);
    const x = Math.min(Math.max(8, event.clientX || 8), window.innerWidth - 300);
    const y = Math.min(Math.max(8, event.clientY || 8), window.innerHeight - 350);
  ...
  function renderBar({ barIndex, sectionKey, beatsData, rhythmData, barInfo, inst, voicingLibrary, onRerender, navMarks }) {
    ...
    head.title = "Cambio 21: clic para marcar ritmo/repetición de este compás.";
    head.tabIndex = 0;
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 21 · Marcas directas en compás + panel izquierdo libre";
    ...
    // Cambio 21: los controles de práctica viven en la Mini consola sesión del panel izquierdo.
  ...
  function mountInRightPanel({ onChordEdit } = {}) {
    ...
    // Cambio 21: NO escondemos los comandos Groove/Play/Metronomo del Main.
