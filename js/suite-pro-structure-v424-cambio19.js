// Studio 936 Composer - Suite Pro Structure / ADN Module v4.2.4 Cambio 19
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV424Cambio19";
...
/* Cambio 19 · Navegación musical visible antes del arreglo */
...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      // Cambio 19: las marcaciones deben quedar arriba, antes del arreglo largo.
      renderNavigationPanel(ctx, root, s, parts);
      renderBuilder(ctx, root, s, parts);
      scheduleRightPanelChart(ctx, root);
    } catch (error) {
      console.error("Suite Pro Structure Cambio 19 render error:", error);
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 19 · Main controla Chart + escenario alto");
  ...
  function scheduleRightPanelChart(ctx, root) {
    ...
    const ensureChartScriptLoaded = () => {
      const src = "js/suite-pro-chart-v218-cambio19.js";
  ...
  function writeNavigationMarks(marks) {
    ...
    try {
      localStorage.setItem(NAV_KEY, JSON.stringify({
        version: "navigation-v1-cambio19",
  ...
  function renderNavigationPanel(ctx, root, s, parts) {
    ...
    card.appendChild(ctx.el("p", "s936-struct-muted", "Define las señales del mapa profesional: repeticiones, Fine, Segno, Coda y casillas. Cambio 19 las deja visibles arriba y dibujadas claramente en el Chart; la obediencia completa en práctica viene después."));
