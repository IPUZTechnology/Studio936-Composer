// Studio 936 Composer - Chart View v2.1.2 Cambio 13 (MINI CONSOLA RÍTMICA CON LUZ POR CELDA)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.1.2-cambio-13";
  const STYLE_ID = "s936-chart-v212-cambio13";
  ...
  function startChartRhythmConsole(container, { withPulse = false } = {}) {
    ...
    const runStep = () => {
      ...
      if (mode === "hit" || mode === "repeat" || mode === "hold") {
        ...
        if (midis.length) schedulePopupRhythmHit(midis, getCurrentChartBpm(), false, mode);
      }
...
.s936-ch-dblbar{height:3px;margin-top:2px;background:linear-gradient(to right,rgba(255,255,255,.15) 0,rgba(255,255,255,.15) calc(100% - 4px),rgba(255,255,255,.5) calc(100% - 4px),rgba(255,255,255,.5) 100%)}
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 13 · Practicar Chart mantiene el acorde durante el compás";
    ...
    // Cambio 13: mini consola ligera para practicar el Chart al BPM e iluminar cada celda.
