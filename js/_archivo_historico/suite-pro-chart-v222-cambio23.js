// Studio 936 Composer - Chart View v2.2.2 Cambio 23 (LOOP FUNCIONAL + MENÚ DE COMPÁS LIMPIO)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.2.2-cambio-23";
  const STYLE_ID = "s936-chart-v222-cambio23";
  ...
  function applyPracticeNavigationToSteps(steps) {
    ...
    if (practiceStart) {
      ...
      if (start >= 0) return list.slice(start).map(step => Object.assign({}, step, { practiceRange: "fromHere" }));
    }
  ...
  function startChartRhythmConsole(container, { withPulse = false } = {}) {
    ...
    _chartRhythmSteps = applyPracticeNavigationToSteps(collectChartRhythmSteps(container));
    ...
    if (sourceEl) {
      ...
      const range = _chartRhythmSteps[0]?.practiceRange === "loop" ? " · Loop activo"
        : (_chartRhythmSteps[0]?.practiceRange === "fromHere" ? " · Desde aquí" : "");
...
.s936-ch-nav-badge[data-nav-type="loopEnd"]{
  ...
  function showBarContextMenu(event, { sectionKey, barIndex, onRerender }) {
    ...
    hint.textContent = "Menú limpio: práctica temporal y repetición local. Las marcas de canción viven en sección/estructura.";
    ...
    addButton(practiceGrid, "▶ Practicar desde aquí", () => setPracticeStartMark(sectionKey, barIndex), "", "Empieza la práctica desde este compás sin crear loop.");
    ...
    addButton(practiceGrid, "Limpiar práctica", () => clearPracticeMarks(sectionKey), "danger", "Quita loop inicio, loop final y practicar desde aquí de esta sección.");

    const barGrid = group("Compás", "rhythm");
    addButton(barGrid, "% Repetir anterior", () => applyBarRhythmPreset(sectionKey, barIndex, "repeatBar"), "warn", "Este compás toma la armonía del compás anterior.");
    addButton(barGrid, "Restaurar compás", () => {
      applyBarRhythmPreset(sectionKey, barIndex, "clearRhythm");
      clearNavigationMarksForBar(sectionKey, barIndex);
    }, "", "Limpia marcas de práctica/compás sin tocar otros compases.");
    ...
    const x = Math.min(Math.max(8, preferredX), window.innerWidth - 300);
    const y = Math.min(Math.max(8, preferredY), window.innerHeight - 300);
  ...
  function renderBar({ barIndex, sectionKey, beatsData, rhythmData, barInfo, inst, voicingLibrary, onRerender, navMarks }) {
    ...
    head.title = "Cambio 23: clic para práctica local o repetir este compás.";
    head.tabIndex = 0;
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 23 · Loop funcional + menú de compás limpio";
    ...
    // Cambio 23: los controles de práctica viven en la Mini consola sesión del panel izquierdo.
