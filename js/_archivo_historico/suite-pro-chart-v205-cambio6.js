// Studio 936 Composer - Chart View v2.0.4 Cambio 5 (INTERACTIVE VOICING EDITOR PRESERVADO)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.0.5-cambio-6";
  const STYLE_ID = "s936-chart-v205-cambio6";
...
.s936-ch-dblbar{height:3px;margin-top:2px;background:linear-gradient(to right,rgba(255,255,255,.15) 0,rgba(255,255,255,.15) calc(100% - 4px),rgba(255,255,255,.5) calc(100% - 4px),rgba(255,255,255,.5) 100%)}
  ...
  function showBeatPop(targetEl, label, currentVal, inst, onSave, onOpenVoicing) {
    closePopups();
    ...
    pop.className = "s936-ch-pop s936-ch-pop-v6";
    const rect = targetEl.getBoundingClientRect();
    const popW = Math.min(360, Math.max(300, Math.floor(window.innerWidth * 0.28)));
    const popH = Math.min(620, Math.max(430, Math.floor(window.innerHeight * 0.56)));
    const left = Math.min(Math.max(8, rect.left), window.innerWidth - popW - 8);
    const topCandidate = rect.bottom + 8;
    const top = topCandidate < window.innerHeight - popH - 8
      ? topCandidate
      : Math.max(8, Math.min(rect.top - popH - 8, window.innerHeight - popH - 8));
    pop.style.cssText = `left:${left}px;top:${top}px;width:${popW}px;max-height:${popH}px;`;
    pop.onclick = (e) => e.stopPropagation();

    const drag = document.createElement("div");
    drag.className = "s936-ch-pop-drag";
    drag.innerHTML = `<span>${label}</span><small>Arrastra esta barra · cambio 6</small>`;
    ...
    pop.appendChild(notesLine);

    const mapLabel = document.createElement("div");
    mapLabel.className = "s936-picker-map-label";
    mapLabel.textContent = "Vista previa instrumental";
    pop.appendChild(mapLabel);

    const mapBox = document.createElement("div");
    mapBox.className = "s936-picker-map-box";
    pop.appendChild(mapBox);

    const hintLine = document.createElement("div");
    hintLine.className = "s936-picker-map-hint";
    hintLine.textContent = previewInst === "piano"
      ? "El piano se recalcula con cada acorde."
      : "El mapa se recalcula con cada acorde. Para poner dedos exactos usa Editar mapa.";
    pop.appendChild(hintLine);
    ...
    function renderPreviewMap(name) {
      mapBox.innerHTML = "";
      if (!name) {
        const empty = document.createElement("div");
        empty.className = "s936-picker-map-empty";
        empty.textContent = "Selecciona nota y calidad para ver el mapa.";
      ...
      notesLine.textContent = pcs.length
        ? "Notas: " + pcs.map(pc => NOTE_NAMES[pc] || "").filter(Boolean).join(" · ")
        : "Notas: —";

      if (previewInst === "piano") {
        mapBox.appendChild(miniPiano(null, name));
      } else {
        const fret = calcFretVoicing(name, previewInst);
        mapBox.appendChild(miniFret(fret));
      }
    }
    ...
    mapBtn.className = "s936-picker-voicing-btn";
    mapBtn.textContent = previewInst === "piano" ? "🎹 Editar mapa de notas" : "🎸 Editar mapa / digitación";
    mapBtn.onclick = (e) => {
      e.stopPropagation();
      const nextName = buildChordName();
      overlay.remove();
      pop.remove();
      if (typeof onOpenVoicing === "function") onOpenVoicing(nextName);
    };
    pop.appendChild(mapBtn);
  ...
  function renderBeat(sectionKey, barIndex, beatIndex, beatVal, inst, voicingLibrary, onRerender) {
    ...
    cell.addEventListener("click", (e) => {
      ...
      showBeatPop(
        ...
        previewName => {
          const startName = previewName || beatVal || "";
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 6 · Popup movible con vista previa instrumental";
