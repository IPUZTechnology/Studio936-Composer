// Studio 936 Composer - Suite Pro Structure / ADN Module v4.6.9 Cambio 65
...
// Cambio 65: ajustes finos al pentagrama — (1) el título siempre muestra los
// nombres de las notas ya puestas (sean 1 o 4); (2) las notas se distribuyen
// de izquierda a derecha según el orden en que se pusieron, para que se lea
// como pentagrama y no como tablatura; (3) etiqueta "Notas de tiempo".
(function () {
  "use strict";
  ...
  const MODULE_CAMBIO_LABEL = "Cambio 65";

  const STYLE_ID = "s936SuiteProStructureStylesV469Cambio65";
  ...
  function buildSequentialStaffPicker(ctx, count, initialPitches, onChange) {
    ...
    NOTE_STAFF_STEPS.forEach((step) => {
      ...
      row.onclick = (e) => {
        ...
        const slot = emptyIdx === -1 ? 0 : emptyIdx; // si ya están las 4, reinicia desde la 1
        pitches[slot] = step.pitch;
        renderDots();
    ...
    function renderDots() {
      ...
      pitches.forEach((p, idx) => {
        ...
        row.appendChild(dot);
        row.title = noteSolfa(p) + (count > 1 ? " · nota " + (idx + 1) : "");
      });
