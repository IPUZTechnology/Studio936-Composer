// Studio 936 Composer - Suite Pro Structure / ADN Module v4.7.5 Cambio 73 (HOTFIX CRÍTICO)
...
// Cambio 73 (HOTFIX CRÍTICO): se encontró y eliminó una regla CSS heredada de
// una versión muy anterior del panel — "display:none!important" sobre
// .s936-lyrics-actions — que ocultaba por completo la barra con el botón
// "✓ Guardar y ver en Chart" y "Cerrar". Un elemento display:none no tiene
// caja de layout, así que el mouse nunca podía tocarlo (por eso el guardado
// solo funcionaba invocándolo por código, nunca con clic real). Diagnosticado
// confirmando que .click() programático SÍ guardaba mientras el clic real no.
(function () {
  "use strict";
  ...
  const MODULE_CAMBIO_LABEL = "Cambio 73";

  const STYLE_ID = "s936SuiteProStructureStylesV475Cambio73";
...
`;
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    saveTop.onclick = (e) => {
      ...
      // Cambio 69: el ícono chico usa el MISMO guardado que el botón grande
      // de abajo (ya reflejaba en el Chart); solo faltaba una confirmación
      // clara para que no pareciera que no hizo nada.
    ...
    boardTop.onclick = (e) => {
      e.stopPropagation();
      saveLyrics(false);
      // Cambio 69: se corrige el mensaje — decía "preparada para tablero",
      // dando a entender que faltaba un paso. En realidad ya se guarda y se
      // refleja en el Chart al instante, igual que el botón grande de abajo.
