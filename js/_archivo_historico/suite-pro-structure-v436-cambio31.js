// Studio 936 Composer - Suite Pro Structure / ADN Module v4.3.6 Cambio 31
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV436Cambio31";
...
`;
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 31 · consola por sección limpia + íconos dorados");
    root.appendChild(banner);
  }

  function scheduleRightPanelChart(ctx, root) {
    const status = ctx.el("section", "s936-struct-right-status");
    status.textContent = "Cambio número 31 · Estructura abrió. Chart + consola por sección…";
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        status.textContent = "Cambio número 31 · Cargando Chart… intento " + (attempt + 1) + ".";
        if (attempt === 0) ensureChartScriptLoaded();
        if (attempt < 12) {
          setTimeout(() => mount(attempt + 1), 160 + attempt * 120);
        } else {
          status.textContent = "Cambio número 31 · No encontré Studio936SuiteProChart.mountInRightPanel(). Revisa que index.html cargue suite-pro-chart-v230-cambio30.js.";
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 31 · Chart montado. Panel izquierdo = consola por sección limpia.";
