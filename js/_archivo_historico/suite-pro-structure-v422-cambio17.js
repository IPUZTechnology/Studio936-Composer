// Studio 936 Composer - Suite Pro Structure / ADN Module v4.2.2 Cambio 17
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV422Cambio17";
  ...
  function register() {
    ...
    window.Studio936SuiteProStructure = { version: "structure-v4.2.2-cambio-17", render };
...
/* Cambio 17 · Navegación musical */
#s936SuitePro .s936-struct-nav-card{
  border-color:rgba(255,224,102,.38);
  background:linear-gradient(135deg,rgba(255,224,102,.10),rgba(0,255,204,.045));
}
...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      renderNavigationPanel(ctx, root, s, parts);
      scheduleRightPanelChart(ctx, root);
    } catch (error) {
      console.error("Suite Pro Structure Cambio 17 render error:", error);
      const err = ctx.el("section", "s936-struct-card dangerzone");
      err.innerHTML = "<h4>Cambio número 17 · Error visible en Estructura</h4><p>No se pudo construir el panel. Revisa la primera línea roja de consola.</p>";
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 17 · Marcaciones musicales profesionales");
    root.appendChild(banner);
  }

  function scheduleRightPanelChart(ctx, root) {
    const status = ctx.el("section", "s936-struct-right-status");
    status.textContent = "Cambio número 17 · Estructura abrió. Chart con marcaciones musicales…";
    root.appendChild(status);

    const ensureChartScriptLoaded = () => {
      const src = "js/suite-pro-chart-v216-cambio17.js";
      const already = Array.from(document.scripts || []).some((script) => String(script.src || "").includes(src));
      if (already) return;
      const script = document.createElement("script");
      script.src = src + "?v=cambio-17-20260628";
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        status.textContent = "Cambio número 17 · Cargando Chart con marcaciones… intento " + (attempt + 1) + ".";
  ...
  function writeNavigationMarks(marks) {
    ...
    try {
      localStorage.setItem(NAV_KEY, JSON.stringify({
        version: "navigation-v1-cambio17",
  ...
  function renderNavigationPanel(ctx, root, s, parts) {
    ...
    card.appendChild(ctx.el("p", "s936-struct-muted", "Define las señales del mapa profesional: repeticiones, Fine, Segno, Coda y casillas. En este cambio se guardan y se dibujan en el Chart; la obediencia completa en práctica viene después."));
