// Studio 936 Composer - Suite Pro Structure / ADN Module v4.3.8 Cambio 33
...
/* Cambio 33 · Limpieza del dock: no trackers/banners temporales */
#s936SuitePro .s936-struct-change-banner,
#s936SuitePro .s936-struct-right-status,
#s936SuitePro .s936-ckpt-topbar,
...
/* Cambio 33 · Dock como consola por sección: menú arriba, botones sobrios y agrupados */
#s936SuitePro .s936-ckpt-topbar,
#s936SuitePro .s936-ckpt-topbar-compact,
#s936SuitePro .s936-ckpt-menu-btn-wide,
#s936SuitePro .s936-ckpt-dropdown{
...
`;
  ...
  function render(ctx, shell) {
    ...
    // Cambio 33: sin banners temporales en el dock; el menú de canción vive en Compose.
    ...
    try {
      ...
      // Cambio 33: el menú de canción sube al nivel Compose; no duplicar cabecera en Estructura.
      ...
      err.innerHTML = "<h4>Cambio número 33 · Error visible en Estructura</h4><p>No se pudo construir el panel. Revisa la primera línea roja de consola.</p>";
  ...
  function renderChangeBanner(ctx, root) { /* Cambio 33: tracker visual removido del dock. */ }

  function scheduleRightPanelChart(ctx, root) {
    const status = ctx.el("section", "s936-struct-right-status");
    status.style.display = "none";
    status.textContent = "Cambio número 33 · Cargando Chart…";
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        status.textContent = "Cambio número 33 · Cargando Chart… intento " + (attempt + 1) + ".";
        if (attempt === 0) ensureChartScriptLoaded();
        if (attempt < 12) {
          setTimeout(() => mount(attempt + 1), 160 + attempt * 120);
        } else {
          status.textContent = "Cambio número 33 · No encontré Studio936SuiteProChart.mountInRightPanel(). Revisa que index.html cargue suite-pro-chart-v232-cambio32.js.";
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 33 · Chart montado. Panel izquierdo = consola por sección limpia.";
