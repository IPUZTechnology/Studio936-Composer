// Studio 936 Composer - Chart View v2.2.1 Cambio 22 (PULIDO VISUAL DE CHART + MENÚ PROFESIONAL)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.2.1-cambio-22";
  const STYLE_ID = "s936-chart-v221-cambio22";
...
/* Cambio 22 · escenario Chart sin borrar los comandos Groove del Main */
...
/* Cambio 22 · menú contextual profesional por compás */
.s936-ch-bar-head{
  cursor:pointer;
  user-select:none;
}
...
/* Cambio 22 · Chart visual pro, sin saturar el panel */
.s936-ch-section-navline{
  display:none!important;
}
.s936-ch-bar{
...
.s936-ch-nav-badge[data-nav-type="loopEnd"]{
  ...
  function renderBeat(sectionKey, barIndex, beatIndex, beatVal, inst, voicingLibrary, onRerender, rhythmMode, inheritedVal) {
    ...
    if (parsed && effectiveRhythm !== "rest") {
      ...
      const chordName = inheritedParsed.root + inheritedParsed.qual;
      const nameUpper = chordName.toUpperCase().trim();
      voicingContainer.classList.add("s936-ch-voicing-held");
      if (inst === "piano") {
        const savedVoicing = voicingLibrary?.[inst]?.[nameUpper] || null;
        voicingContainer.appendChild(miniPiano(savedVoicing, chordName));
      } else {
        const fretVoicing = voicingLibrary?.[inst]?.[nameUpper] || calcFretVoicing(chordName, inst);
        voicingContainer.appendChild(miniFret(fretVoicing));
      }
    } else if (rInfo.symbol) {
  ...
  function showBarContextMenu(event, { sectionKey, barIndex, onRerender }) {
    ...
    menu.className = "s936-bar-menu s936-bar-menu-pro";
    ...
    const hint = document.createElement("div");
    hint.className = "s936-bar-menu-hint";
    hint.textContent = "Marca este compás sin llenar el panel lateral. Canción = partitura; práctica = loop temporal.";
    menu.appendChild(hint);

    function group(label, cls = "") {
      const g = document.createElement("div");
      g.className = "s936-bar-menu-group " + cls;
    ...
    function addButton(grid, label, fn, cls, titleText) {
      const btn = document.createElement("button");
      btn.type = "button";
    ...
    const rhythmGrid = group("Ritmo del compás", "rhythm");
    addButton(rhythmGrid, "♩ Tocar + sostener", () => applyBarRhythmPreset(sectionKey, barIndex, "hitHold"), "", "Beat 1 ataca; beats 2-4 sostienen");
    addButton(rhythmGrid, "% Repetir anterior", () => applyBarRhythmPreset(sectionKey, barIndex, "repeatBar"), "warn", "Repite el compás anterior");
    addButton(rhythmGrid, "Restaurar ritmo", () => applyBarRhythmPreset(sectionKey, barIndex, "clearRhythm"), "", "Quita marcas rítmicas del compás");

    const scoreGrid = group("Partitura", "score");
    addButton(scoreGrid, "𝄆 Inicio", () => upsertNavigationMark(sectionKey, barIndex, "repeatStart", 2));
    addButton(scoreGrid, "𝄇 Fin x2", () => upsertNavigationMark(sectionKey, barIndex, "repeatEnd", 2), "warn");
    addButton(scoreGrid, "Fine", () => upsertNavigationMark(sectionKey, barIndex, "fine", 2), "stop");
    addButton(scoreGrid, "𝄋 Segno", () => upsertNavigationMark(sectionKey, barIndex, "segno", 2));
    addButton(scoreGrid, "Coda", () => upsertNavigationMark(sectionKey, barIndex, "coda", 2));

    const practiceGrid = group("Práctica", "practice");
    addButton(practiceGrid, "Loop inicio", () => upsertNavigationMark(sectionKey, barIndex, "loopStart", 2));
    addButton(practiceGrid, "Loop final", () => upsertNavigationMark(sectionKey, barIndex, "loopEnd", 2));
    addButton(practiceGrid, "Limpiar loop", () => clearPracticeLoopMarks(sectionKey), "danger");

    const cleanGrid = group("Limpiar compás", "clean");
    addButton(cleanGrid, "Quitar marcas", () => clearNavigationMarksForBar(sectionKey, barIndex), "danger");
    ...
    const x = Math.min(Math.max(8, preferredX), window.innerWidth - 340);
    const y = Math.min(Math.max(8, preferredY), window.innerHeight - 420);
  ...
  function renderBar({ barIndex, sectionKey, beatsData, rhythmData, barInfo, inst, voicingLibrary, onRerender, navMarks }) {
    ...
    head.title = "Cambio 22: clic para marcar ritmo/repetición de este compás.";
    head.tabIndex = 0;
    ...
    fig.title = "Clic para ritmo/marcas de este compás";
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 22 · Marcas directas en compás + panel izquierdo libre";
    ...
    // Cambio 22: los controles de práctica viven en la Mini consola sesión del panel izquierdo.
  ...
  function mountInRightPanel({ onChordEdit } = {}) {
    ...
    // Cambio 22: NO escondemos los comandos Groove/Play/Metronomo del Main.
