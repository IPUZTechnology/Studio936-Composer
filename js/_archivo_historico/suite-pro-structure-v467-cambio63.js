// Studio 936 Composer - Suite Pro Structure / ADN Module v4.6.7 Cambio 63
...
// Cambio 63: badge de versión visible en el título del panel Ly Letra (ej.
// "CAMBIO 63"), para confirmar de un vistazo si el navegador está corriendo
// la última versión o una anterior en caché — evita confundir bugs viejos
// ya corregidos con problemas nuevos.
(function () {
  "use strict";
  ...
  const MODULE_CAMBIO_LABEL = "Cambio 63";

  const STYLE_ID = "s936SuiteProStructureStylesV467Cambio63";
...
/* Cambio 62 · marca las notas de las OTRAS sílabas del mismo tresillo/cuatrillo
   directamente sobre el pentagrama, para ver el grupo completo dibujado ahí. */
.s936-note-row .s936-note-sibling{
  position:absolute;
  left:50%;
  top:50%;
  transform:translate(-50%,-50%);
  ...
  border-radius:50%;
  background:rgba(0,255,204,.45);
  box-shadow:0 0 0 2px rgba(4,7,10,.9);
  pointer-events:none;
}
...
`;
  ...
  function buildNoteStaffPicker(ctx, initialPitch, onPick, secondaryPitches) {
    ...
    const secondarySet = new Set((secondaryPitches || []).filter(Boolean));
    NOTE_STAFF_STEPS.forEach((step) => {
      ...
      // Cambio 62: si este tiempo está dividido (tresillo/cuatrillo), las
      // notas de las OTRAS sílabas se marcan aquí mismo, atenuadas, para ver
      // el grupo completo dibujado sobre el pentagrama, no solo la actual.
      if (secondarySet.has(step.pitch)) row.appendChild(ctx.el("span", "s936-note-sibling"));
      row.onclick = (e) => {
        e.stopPropagation();
      ...
      for (let b = 0; b < 4; b++) {
        ...
        function renderSubdivision(n, presetTexts, presetPitches) {
          ...
          for (let s = 0; s < n; s++) {
            ...
            // Cambio 62: el pentagrama de cada sílaba se reconstruye justo
            // al abrirlo, marcando las notas de las OTRAS sílabas del mismo
            // tresillo/cuatrillo — así se ve el grupo completo dibujado
            // sobre el plano del pentagrama, no encerrado en la celda de texto.
            function rebuildMiniStaff() {
              miniPop.innerHTML = "";
              const siblingPitches = subInputs
                .map((si, idx) => (idx === s ? null : si.getPitch()))
                .filter(Boolean);
