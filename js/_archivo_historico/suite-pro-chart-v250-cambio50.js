// Studio 936 Composer - Chart View v2.5.0 Cambio 50 (LYRICS CLEAN KARAOKE)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.5.0-cambio-50";
  const STYLE_ID = "s936-chart-v250-cambio50";
  ...
  function readFocusSection() {
    ...
    try {
      ...
      if (raw && raw.active && raw.section) return raw;
    } catch(_) {}
    return null;
  }
...
`;
    s.textContent += `
/* Cambio 45 · lyrics por compás visibles en Chart */
.s936-ch-lyric-line{
  margin:4px 4px 2px;
  padding:5px 7px;
  border-radius:9px;
  border:1px solid rgba(255,224,102,.20);
  background:linear-gradient(90deg,rgba(255,224,102,.07),rgba(0,255,204,.045));
...
.s936-ch-bar.s936-cb-active .s936-ch-lyric-line,
.s936-ch-beat.active-beat ~ .s936-ch-lyric-line{
  border-color:rgba(0,255,204,.52);
  color:#eafffb;
...
/* Cambio 48 · lyrics más grandes y por tiempos en Chart */
.s936-ch-lyric-line{
  margin:7px 5px 3px!important;
  padding:7px 8px!important;
  border-radius:10px!important;
  border-color:rgba(0,255,204,.22)!important;
  background:linear-gradient(90deg,rgba(0,255,204,.07),rgba(255,224,102,.05))!important;
...
.s936-ch-beat.active-beat ~ .s936-ch-lyric-line,
.s936-ch-bar.s936-cb-active .s936-ch-lyric-line{
  box-shadow:0 0 18px rgba(0,255,204,.20)!important;
}

`;

    s.textContent += `
/* Cambio 50 · lyrics limpios tipo karaoke, sin números y también en compases % */
.s936-ch-lyric-line{
  margin:8px 6px 4px!important;
  padding:8px 10px!important;
  border-radius:11px!important;
  border-color:rgba(0,255,204,.18)!important;
  background:linear-gradient(90deg,rgba(0,255,204,.055),rgba(255,224,102,.035))!important;
}
.s936-ch-lyric-beats{
  gap:5px!important;
}
.s936-ch-lyric-beat{
  ...
  font-weight:950!important;
  letter-spacing:.02em;
  border-color:rgba(0,255,204,.12)!important;
...
.s936-ch-lyric-beat.has-text{
  ...
  background:rgba(0,255,204,.075)!important;
  border-color:rgba(0,255,204,.24)!important;
}
.s936-ch-repeat-bar .s936-ch-lyric-line{
  width:min(90%,560px);
  margin:10px auto 0!important;
}
.s936-ch-lyric-beat.is-long{
  position:relative;
  box-shadow:inset 0 -2px 0 rgba(255,224,102,.42);
}
...
`;
  ...
  function renderBar({ barIndex, sectionKey, beatsData, rhythmData, barInfo, inst, voicingLibrary, onRerender, navMarks }) {
    ...
    if (barInfo?.chord && !barInfo?.isFirst && !hasExplicitBeatInBar) {
      ...
      if (lyricRepeatEl) rep.appendChild(lyricRepeatEl);
      return bar;
    }
