// Studio 936 Composer - Suite Pro Structure / ADN Module v4.6.4 Cambio 60 (HOTFIX CRÍTICO)
...
// Cambio 60 (HOTFIX CRÍTICO): el Cambio 59 llamaba syncMiniStaffPreview() ANTES
// de declarar "const subdivideBtns", lo que lanzaba un ReferenceError (temporal
// dead zone) en la primera celda de cada sección y rompía TODA la construcción
// del editor — por eso Ly Letra dejaba de abrir. Se reordenó la llamada.
(function () {
  "use strict";

  const STYLE_ID = "s936SuiteProStructureStylesV464Cambio60";
...
`;
  ...
  // Cambio 59: mini-pentagrama de vista previa — dibuja las notas ya grabadas
  // (una sola, o el tresillo/cuatrillo completo) directamente sobre 5 líneas,
  // visible en la celda aunque el popover esté cerrado.
  function buildMiniStaffPreview(ctx) {
    const box = ctx.el("div", "s936-lyric-mini-staff");
    const linesEl = ctx.el("div", "s936-lyric-mini-staff-lines");
    [0, 25, 50, 75, 100].forEach((pct) => {
      const line = ctx.el("span", "s936-lyric-mini-staff-line");
      line.style.top = pct + "%";
      linesEl.appendChild(line);
    });
    const notesEl = ctx.el("div", "s936-lyric-mini-staff-notes");
    box.append(linesEl, notesEl);

    function render(pitches) {
      notesEl.innerHTML = "";
      const list = (pitches || []).filter(Boolean);
      box.classList.toggle("is-empty", list.length === 0);
      const n = list.length;
      list.forEach((pitch, idx) => {
        const stepIndex = NOTE_STAFF_STEPS.findIndex((s) => s.pitch === pitch);
        if (stepIndex < 0) return;
        const dot = ctx.el("span", "s936-lyric-mini-note");
        dot.style.top = (stepIndex / (NOTE_STAFF_STEPS.length - 1) * 100) + "%";
        dot.style.left = (((idx + 0.5) / Math.max(n, 1)) * 100) + "%";
        dot.title = noteSolfa(pitch);
        notesEl.appendChild(dot);
      });
    }
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    for (let i = 0; i < bars; i++) {
      ...
      for (let b = 0; b < 4; b++) {
        ...
        const miniStaffPreview = buildMiniStaffPreview(ctx); // Cambio 59
        ...
        // Cambio 59: recalcula qué notas mostrar en el mini-pentagrama de la
        // celda — una sola (tiempo normal) o el tresillo/cuatrillo completo
        // (tiempo dividido), tal como quedaron grabadas.
        function syncMiniStaffPreview() {
          const currentSubdivision = Number(subdivideBtns?.querySelector(".is-active")?.dataset.value || initialSubdivision);
          if (currentSubdivision > 1) {
            miniStaffPreview.render(subInputs.map((si) => si.getPitch()));
          } else {
            miniStaffPreview.render(durationWrap.dataset.pitch ? [durationWrap.dataset.pitch] : []);
          }
        }
        ...
        function renderSubdivision(n, presetTexts, presetPitches) {
          ...
          for (let s = 0; s < n; s++) {
            ...
            miniDot.onclick = (e) => {
              ...
              if (wasHidden) openLyricPopover(miniPop);
            };
        ...
        // Cambio 60 (HOTFIX): esta llamada inicial estaba ANTES de que
        // subdivideBtns existiera (const subdivideBtns se declaraba después),
        // lo que lanzaba un ReferenceError (temporal dead zone) en la primera
        // celda de CADA sección — y eso rompía la construcción completa del
        // editor, por lo que Ly Letra dejaba de abrir. Se movió aquí, después
        // de que subdivideBtns ya existe.
        syncMiniStaffPreview();
        ...
        durationWrap.append(miniStaffPreview.el, durationBtn, durationPop);
