// Studio 936 Composer - Suite Pro Structure / ADN Module v4.4.6 Cambio 42
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV447Cambio42";
  ...
  function installDockFlexGuard() {
    ...
    const sync = () => {
      try {
        ...
        if (chartPanel) {
          // Cambio 42: no empujar el Chart con margin-left; eso generaba scroll horizontal
  ...
  function installChartStageKeeperCambio41() {
    ...
    const ensure = () => {
      try {
        ...
        console.warn("Cambio 42 chart keeper:", error);
      }
    };
...
`;
    ...
    style.textContent += `
/* Cambio 42 · Dock flexible real, sin cubrir Chart ni crear scroll horizontal */
html, body{
  overflow-x:hidden!important;
}
#s936SuitePro{
...
`;
    ...
    style.textContent += `
/* Cambio 42 · Zoom sección: consola ampliada de sesión */
...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      err.innerHTML = "<h4>Cambio número 37 · Cambio número 42 · Error visible en Estructura</h4><p>No se pudo construir el panel. Revisa la primera línea roja de consola.</p>";
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        ...
        if (attempt < 12) {
          ...
          status.textContent = "Cambio número 37 · No encontré Studio936SuiteProChart.mountInRightPanel(). Revisa que index.html cargue suite-pro-chart-v242-cambio42.js.";
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 37 · Chart montado. Panel izquierdo = consola por sección limpia.";
  ...
  function renderZoomSessionConsole(ctx, root, s, part, parts) {
    ...
    title.appendChild(ctx.el("small", "", label + " · trabajo rápido de la sección"));
    const badge = ctx.el("span", "s936-zoom-session-badge", "Zoom activo");
    head.append(title, badge);
    card.appendChild(head);

    const grid = ctx.el("div", "s936-zoom-session-grid");

    const play = ctx.el("button", "s936-zoom-session-btn primary", "▶ Play sección");
    const loop = ctx.el("button", "s936-zoom-session-btn", "↻ Loop sección");
    const stop = ctx.el("button", "s936-zoom-session-btn danger", "■ Stop");
    const lyric = ctx.el("button", "s936-zoom-session-btn", "Ly Letra");
    const recVoice = ctx.el("button", "s936-zoom-session-btn future", "REC voz");
    const recInst = ctx.el("button", "s936-zoom-session-btn future", "REC instrumento");
    const midi = ctx.el("button", "s936-zoom-session-btn future", "MIDI / Teclado");
    const audioIn = ctx.el("button", "s936-zoom-session-btn future", "Audio / Interfaz");

    const status = ctx.el("div", "s936-zoom-session-status", "Lista para trabajar esta sección. Play/Loop usan el Chart; REC, MIDI y Audio quedan preparados para el módulo Studio.");
    const panel = () => document.getElementById("s936-chart-view-panel");
    ...
    play.onclick = (e) => {
      e.stopPropagation();
      const ok = window.Studio936SuiteProChart?.startChartSectionPractice?.(panel(), section, { withPulse:false, sourceLabel:"Zoom sección" });
      setConsoleChannelActive(section, "zoom-play");
      setStatus(ok ? "Reproduciendo sección: " + label : "El Chart aún no está listo para reproducir esta sección.");
    };
    loop.onclick = (e) => {
      e.stopPropagation();
      try {
        window.dispatchEvent(new CustomEvent("studio936:chart-loop-current-section", {
          detail: { section, part: label, source: "zoom-session-console" }
        }));
      } catch(_) {}
      const ok = window.Studio936SuiteProChart?.startChartSectionPractice?.(panel(), section, { withPulse:false, sourceLabel:"Loop zoom sección" });
      setConsoleChannelActive(section, "zoom-loop");
      setStatus(ok ? "Loop activo para la sección: " + label : "Loop preparado; el Chart aún no confirmó práctica.");
    };
    stop.onclick = (e) => {
      e.stopPropagation();
      ...
      clearConsoleChannelActive();
      setStatus("Sesión detenida.");
    };
    lyric.onclick = (e) => {
      e.stopPropagation();
      try {
        window.dispatchEvent(new CustomEvent("studio936:open-section-lyrics", { detail: { section, label } }));
      } catch(_) {}
      setStatus("Letra por compás preparada para: " + label + ". Próximo módulo: editor Ly/Karaoke.");
    };

    function future(labelText, eventName) {
      return (e) => {
        e.stopPropagation();
        try {
          window.dispatchEvent(new CustomEvent(eventName, { detail: { section, label, bpm, style, source: "zoom-session-console" } }));
        } catch(_) {}
        setStatus(labelText + " preparado para esta sección. El envío real se conectará al módulo Studio.");
      };
    }
    recVoice.onclick = future("REC voz", "studio936:prepare-section-voice-rec");
    recInst.onclick = future("REC instrumento", "studio936:prepare-section-instrument-rec");
    midi.onclick = future("MIDI / teclado", "studio936:prepare-section-midi");
    audioIn.onclick = future("Audio / interfaz", "studio936:prepare-section-audio-input");

    grid.append(play, loop, stop, lyric, recVoice, recInst, midi, audioIn);
    card.appendChild(grid);

    const block = ctx.el("div", "s936-zoom-session-block");
    block.appendChild(ctx.el("h5", "", "Conexiones rápidas"));
    const chips = ctx.el("div", "s936-zoom-session-chips");
    ["Micrófono", "Guitarra / línea", "Interfaz de audio", "Teclado MIDI", "Monitor"].forEach(text => {
      chips.appendChild(ctx.el("span", "s936-zoom-session-chip", text));
    });
  ...
  function renderBuilder(ctx, root, s, parts) {
    ...
    if (focusedPart) {
      const focusbar = ctx.el("div", "s936-ckpt-focusbar");
      const label = focusedPart.label || labelFor(focusedPart.section);
      const info = ctx.el("div", "");
      info.appendChild(ctx.el("b", "", "Zoom sección · " + label));
      info.appendChild(ctx.el("small", "", "Chart filtrado y consola enfocada en esta sección."));
      const exitBtn = ctx.el("button", "", "Salir zoom");
      exitBtn.onclick = () => {
        state.focusSection = "";
        saveState();
      ...
      focusbar.append(info, exitBtn);
      listCard.appendChild(focusbar);
    }
  ...
  function partRow(ctx, s, parts, part, index) {
    ...
    lyricBtn.onclick = (e) => {
      e.stopPropagation();
      alert("Próximo módulo: letra por compás para karaoke de la sección " + (part.label || labelFor(part.section)) + ".");
    };
