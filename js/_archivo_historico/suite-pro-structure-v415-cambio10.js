// Studio 936 Composer - Suite Pro Structure / ADN Module v4.1.4 Cambio 9
...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      err.innerHTML = "<h4>Cambio número 10 · Error visible en Estructura</h4><p>No se pudo construir el panel. Revisa la primera línea roja de consola.</p>";
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 10 · Ritmo en vivo dentro del popup");
    root.appendChild(banner);
  }

  function scheduleRightPanelChart(ctx, root) {
    const status = ctx.el("section", "s936-struct-right-status");
    status.textContent = "Cambio número 10 · Estructura abrió. Chart + ritmo vivo activo…";
    root.appendChild(status);

    const ensureChartScriptLoaded = () => {
      const src = "js/suite-pro-chart-v209-cambio10.js";
      const already = Array.from(document.scripts || []).some((script) => String(script.src || "").includes(src));
      if (already) return;
      const script = document.createElement("script");
      script.src = src + "?v=cambio-10-20260628";
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        status.textContent = "Cambio número 10 · Cargando Chart para panel derecho… intento " + (attempt + 1) + ".";
        if (attempt === 0) ensureChartScriptLoaded();
        if (attempt < 12) {
          setTimeout(() => mount(attempt + 1), 160 + attempt * 120);
        } else {
          status.textContent = "Cambio número 10 · No encontré Studio936SuiteProChart.mountInRightPanel(). Revisa que index.html cargue suite-pro-chart-v209-cambio10.js.";
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 10 · Chart montado en el panel derecho. Selector: Piano · Guitarra · Ukulele · Bajo.";
          status.style.color = "#bfffee";
        } else {
          status.textContent = "Cambio número 10 · Chart no pudo montarse en el panel derecho: " + (result?.reason || "sin razón reportada") + ".";
          status.style.color = "#ffb5b5";
        }
      } catch (error) {
        console.warn("Suite Pro Structure Cambio 9 Chart right panel:", error);
        status.textContent = "Cambio número 10 · Error al montar Chart en panel derecho. Revisa la primera línea roja de consola.";
        status.style.color = "#ffb5b5";
      }
    };
