// Studio 936 Composer - Chart View v2.1.6 Cambio 17 (MARCACIONES MUSICALES)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.1.6-cambio-17";
  const STYLE_ID = "s936-chart-v216-cambio17";
  ...
  function scheduleChartPracticeGroove(step, mode, bridgeState) {
    ...
    for (let sub = 0; sub < 4; sub += 1) {
      ...
      if (Array.isArray(rhythm.bass) && rhythm.bass.includes(step16)) {
        ...
        playEngineNote(bass, 0.28, normalizedMode === "hold" ? 0.34 : 0.44, "sine", offset);
        if (step16 === 0 || styleName === "rock" || styleName === "ballad") {
          playEngineNote(Math.max(24, bass - 12), 0.16, normalizedMode === "hold" ? 0.40 : 0.56, "sine", offset);
        }
      }
      if (rhythm.arp) {
        ...
        if (arpSteps.includes(step16)) {
          ...
          playEngineNote(m, 0.13, normalizedMode === "hold" ? 0.38 : 0.52, "triangle", offset);
        }
      }
      if (Array.isArray(rhythm.chord) && rhythm.chord.includes(step16)) {
        playEngineChord(chordMidis, 0.13, normalizedMode === "hold" ? 0.23 : 0.34, offset);
      }
      if (Array.isArray(rhythm.ghost) && rhythm.ghost.includes(step16)) {
        playEngineChord(thinChartChord(chordMidis), 0.055, 0.16, offset);
      }
    }
    ...
    if (!hasAny && normalizedMode === "hold") {
      playEngineChord(chordMidis, Math.min(0.55, 60 / bpm * 0.85), 0.18, 0);
    }
    return true;
  }
  ...
  function startChartRhythmConsole(container, { withPulse = false } = {}) {
    ...
    if (sourceEl) {
      ...
      sourceEl.textContent = bridgeState.source === "audio-engine"
        ? "Motor: AudioEngine + Rhythm · BPM " + bridgeState.bpm
...
.s936-ch-section-navline .s936-ch-nav-badge{
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 17 · Marcaciones musicales visibles en Chart";
