// Studio 936 Composer - Chart View v2.0.3 Cambio 4 (INTERACTIVE VOICING EDITOR PRESERVADO)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.0.3-cambio-4";
  const STYLE_ID = "s936-chart-v203-cambio4";
...
.s936-ch-change-banner{
  ...
  background:linear-gradient(90deg,rgba(0,255,204,.20),rgba(255,91,234,.16));
  border-bottom:1px solid rgba(0,255,204,.32);
  color:#dffff8;font-size:.68rem;font-weight:950;text-transform:uppercase;
  letter-spacing:.9px
}
...
.s936-ch-dblbar{height:3px;margin-top:2px;background:linear-gradient(to right,rgba(255,255,255,.15) 0,rgba(255,255,255,.15) calc(100% - 4px),rgba(255,255,255,.5) calc(100% - 4px),rgba(255,255,255,.5) 100%)}
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    if (!container) return;
    installStyles();
    // Cambio 4: si el panel todavía no está pegado al DOM, reintentar un ciclo después.
    ...
    changeBanner.textContent = "Cambio número 4 · Chart v2 preservado, montado después del panel";
