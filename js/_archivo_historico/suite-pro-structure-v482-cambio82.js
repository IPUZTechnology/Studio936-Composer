// Studio 936 Composer - Suite Pro Structure / ADN Module v4.8.2 Cambio 82
...
// Cambio 82: el panel Ly Letra se ancla al FONDO de la ventana (ya no queda
// pegado arriba) — el "top" se calcula a partir de la altura real del
// contenido, así los primeros compases del Chart quedan visibles por
// encima del panel en vez de taparlos.
(function () {
  "use strict";
  ...
  const MODULE_CAMBIO_LABEL = "Cambio 82";

  const STYLE_ID = "s936SuiteProStructureStylesV482Cambio82";
...
.s936-lyrics-float-panel{
  /* Cambio 68: se quita el !important del ancho — con !important, ganaba
     siempre sobre el estilo inline que el navegador aplica al arrastrar la
     esquina de redimensionar, y por eso el panel solo crecía verticalmente
     (el alto sí podía cambiar porque no tenía !important). Ahora resize:both
     funciona en las dos direcciones, igual que ya funcionaba el alto. */
  width:min(1180px,calc(100vw - 34px));
...
`;
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    try {
      ...
      // Cambio 81: se mide la altura real del contenido (cabecera + compases)
      // en vez de siempre llenar la ventana, para no dejar un hueco vacío
      // cuando la sección tiene pocos compases.
      const minTop = 96;
      const maxHeight = Math.max(300, window.innerHeight - minTop - 16);
      const contentHeight = head.offsetHeight + body.scrollHeight + 6;
      const height = Math.min(maxHeight, Math.max(300, contentHeight));
      // Cambio 82: el panel se ancla al FONDO de la ventana (no queda pegado
      // arriba) — así los primeros compases del Chart quedan visibles por
      // encima del panel, en vez de taparlos.
      const bottomMargin = 16;
      const top = Math.max(minTop, window.innerHeight - height - bottomMargin);
