// Studio 936 Composer - Chart View v2.0.6 Cambio 7 (POPUP ÚNICO CON MAPA EDITABLE)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.0.6-cambio-7";
  const STYLE_ID = "s936-chart-v206-cambio7";
...
/* ─── CAMBIO 7: popup único, ancho, mapa editable directo ─── */
.s936-ch-pop-v7{
  resize:both;
  min-width:520px;
  min-height:380px;
...
.s936-ch-dblbar{height:3px;margin-top:2px;background:linear-gradient(to right,rgba(255,255,255,.15) 0,rgba(255,255,255,.15) calc(100% - 4px),rgba(255,255,255,.5) calc(100% - 4px),rgba(255,255,255,.5) 100%)}
  ...
  function showBeatPop(targetEl, label, currentVal, inst, onSave, onOpenVoicing) {
    ...
    pop.className = "s936-ch-pop s936-ch-pop-v7";
    const rect = targetEl.getBoundingClientRect();
    const popW = Math.min(760, Math.max(560, Math.floor(window.innerWidth * 0.42)));
    const popH = Math.min(560, Math.max(420, Math.floor(window.innerHeight * 0.62)));
    ...
    drag.innerHTML = `<span>${label}</span><small>Arrastra · cambio 7</small>`;
    ...
    function currentInlineVoicing() {
      if (previewInst === "piano") {
        const midis = Array.isArray(inlineNotes)
          ...
          : [];
        return midis.length ? { midis, source: "popup-inline-v7" } : null;
      }
      if (!Array.isArray(inlineFrets)) return null;
      const hasAny = inlineFrets.some(f => f !== null && f !== undefined && String(f).toUpperCase() !== "X");
      return hasAny ? { frets: [...inlineFrets], startFret: fretStart, source: "popup-inline-v7" } : null;
    }
    ...
    const doSave = (val, voicing) => { pop.remove(); onSave(val, voicing); };
    okBtn.onclick = (e) => { e.stopPropagation(); doSave(buildChordName(), currentInlineVoicing()); };
    delBtn.onclick = (e) => { e.stopPropagation(); doSave("", null); };
    ...
    document.addEventListener("keydown", function esc(e) {
      if (e.key === "Escape") { pop.remove(); document.removeEventListener("keydown", esc); }
    });
  ...
  function renderBeat(sectionKey, barIndex, beatIndex, beatVal, inst, voicingLibrary, onRerender) {
    ...
    hint.textContent = "🖱 Click para editar acorde y mapa";
    cell.appendChild(hint);

    cell.addEventListener("dblclick", (e) => {
      e.stopPropagation();
      // Cambio 7: no abrir tercera ventana; el mapa se edita dentro del mismo popup de acorde.
    });
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 7 · Popup único con mapa editable directo";
