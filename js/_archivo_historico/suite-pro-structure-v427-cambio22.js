// Studio 936 Composer - Suite Pro Structure / ADN Module v4.2.6 Cambio 22
...
/* Cambio 22 · Navegación musical visible antes del arreglo */
...
/* Cambio 22 · Mini consola sesión real; las marcas se editan en el Chart */
...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      // Cambio 22: las marcaciones se editan directamente en el Chart; el panel izquierdo queda para consola.
      renderSessionConsole(ctx, root, s, parts);
      renderBuilder(ctx, root, s, parts);
      scheduleRightPanelChart(ctx, root);
    } catch (error) {
      console.error("Suite Pro Structure Cambio 22 render error:", error);
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 22 · Mini consola sesión + Chart limpio");
  ...
  function renderNavigationPanel(ctx, root, s, parts) {
    ...
    card.appendChild(ctx.el("p", "s936-struct-muted", "Define las señales del mapa profesional: repeticiones, Fine, Segno, Coda y casillas. Cambio 22 las deja visibles arriba y dibujadas claramente en el Chart; la obediencia completa en práctica viene después."));
