// Studio 936 Composer - Suite Pro Structure / ADN Module v4.0.9 Cambio 4
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV409Cambio4";
  ...
  function register() {
    ...
    window.Studio936SuiteProStructure = { version: "structure-v4.0.9-cambio-4", render };
...
#s936SuitePro .s936-struct-change-banner{
  ...
  border-radius:14px;
  background:linear-gradient(90deg,rgba(0,255,204,.17),rgba(255,91,234,.12));
  color:#dffff8;
  padding:9px 12px;
  font-size:.70rem;
  font-weight:950;
  letter-spacing:.8px;
  text-transform:uppercase;
  box-shadow:0 0 22px rgba(0,255,204,.08);
}
...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      scheduleInlineChart(ctx, root);
    } catch (error) {
      console.error("Suite Pro Structure Cambio 4 render error:", error);
      const err = ctx.el("section", "s936-struct-card dangerzone");
      err.innerHTML = "<h4>Cambio número 4 · Error visible en Estructura</h4><p>No se pudo construir el panel. Revisa la primera línea roja de consola.</p>";
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 4 · Estructura se monta primero y luego carga Chart");
    root.appendChild(banner);
  }

  function scheduleInlineChart(ctx, root) {
    const placeholder = ctx.el("section", "s936-struct-card s936-struct-chart-card");
    placeholder.dataset.s936ChartCard = "cambio4";
    const title = ctx.el("div", "s936-struct-chart-title");
    title.appendChild(ctx.el("h4", "", "Mapa de la canción / Chart View"));
    title.appendChild(ctx.el("span", "", "Selector: Piano · Guitarra · Ukulele · Bajo"));
    placeholder.appendChild(title);

    const panel = ctx.el("div", "");
    panel.id = "s936-chart-view-panel";
    panel.innerHTML = '<p style="color:rgba(255,255,255,.62);padding:18px;text-align:center;font-size:.72rem;line-height:1.45">Cambio número 4 · Preparando Chart dentro de Estructura…</p>';
    placeholder.appendChild(panel);
    root.appendChild(placeholder);

    const run = () => renderInlineChart(ctx, panel);
  ...
  function renderInlineChart(ctx, panel) {
    // Cambio 4: antes de crear el panel inline, desmontar solo cualquier Chart flotante viejo.
    try { window.Studio936SuiteProChart?.unmountFromRightPanel?.(); } catch(_) {}

    const status = (msg, tone) => {
      const color = tone === "error" ? "#ffb5b5" : "rgba(255,255,255,.62)";
      return `<p style="color:${color};padding:18px;text-align:center;font-size:.72rem;line-height:1.45">${msg}</p>`;
    };

    const ensureChartScriptLoaded = () => {
      const src = "js/suite-pro-chart-v203-cambio4.js";
      const already = Array.from(document.scripts || []).some((s) => String(s.src || "").includes(src));
      if (already) return;
      const script = document.createElement("script");
      script.src = src + "?v=cambio-4-20260628";
      script.dataset.s936Cambio4ChartLoader = "true";
      document.body.appendChild(script);
    };

    const paintChart = (attempt = 0) => {
      const Chart = window.Studio936SuiteProChart;

      if (!Chart || typeof Chart.render !== "function") {
        panel.innerHTML = status("Cambio número 4 · Cargando Chart View dentro de Estructura… intento " + (attempt + 1) + ".", "wait");
        if (attempt === 0) ensureChartScriptLoaded();
        if (attempt < 10) {
          setTimeout(() => paintChart(attempt + 1), 160 + attempt * 130);
        } else {
          panel.innerHTML = status("Cambio número 4 · Chart View no está registrado. Revisa que exista js/suite-pro-chart-v203-cambio4.js y que index.html lo cargue antes de Estructura.", "error");
        }
        return;
      }

      const onEdit = (sectionKey, chordIndex) => {
        try {
      ...
      try {
        Chart.render({
          container: panel,
          instrument: localStorage.getItem("s936_chart_inst_v1") || window.Studio936AppBridge?.getEditorState?.()?.instrument || "piano",
          onChordEdit: onEdit
        });
      } catch (error) {
        console.warn("Suite Pro Structure Chart inline cambio 4:", error);
        panel.innerHTML = status("Cambio número 4 · Error al renderizar Chart View. Abre consola y revisa la primera línea roja.", "error");
      }
    };
