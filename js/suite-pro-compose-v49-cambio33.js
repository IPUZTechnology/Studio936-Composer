// Studio 936 Composer - Suite Pro Compose Coordinator v4.9 Cambio 33
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProComposeStylesCambio33";
...
/* Cambio 33 · Dock limpio: sin banners temporales; menú de canción en nivel Compose */
...
/* Cambio 33 · Menú principal como tercer item junto a COMPOSE / STUDIO */
...
`;
  ...
  function render(ctx) {
    ...
    // Cambio 33: el menú de canción sube al nivel Compose; se quitan banners temporales del dock.
    ...
    try {
      ...
      box.innerHTML = "<b>Cambio número 33 · Estructura no pudo renderizar.</b><br>El error queda visible en pantalla. Revisa la primera línea roja de consola.";
  ...
  function renderComposeSongMenu(ctx, shell) {
    // Cambio 33: el menú sube al nivel de COMPOSE / STUDIO como tercer ítem.
    // Mapa Maestro se conserva dentro del submenú Compose, no como pestaña principal.
    setTimeout(() => {
      try {
        mountComposeTopMenu();
        scheduleMasterMapRelocation();
      } catch (err) {
        console.warn("Cambio 33 menú Compose:", err);
  ...
  function buildComposeMenuWrap() {
    ...
    dd.appendChild(item("Mapa Maestro / Mapa de canción", () => {
