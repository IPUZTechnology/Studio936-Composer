// Studio 936 Composer - Suite Pro Structure / ADN Module v4.5.0 Cambio 45
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV450Cambio45";
  ...
  function installDockFlexGuard() {
    ...
    const sync = () => {
      try {
        ...
        if (chartPanel) {
          // Cambio 45: no empujar el Chart con margin-left; eso generaba scroll horizontal
...
`;
    ...
    style.textContent += `
/* Cambio 45 · Dashboard visual + editor Ly por compás */
#s936SuitePro .s936-zoom-session-console{
...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      err.innerHTML = "<h4>Cambio número 45 · Error visible en Estructura</h4><p>No se pudo construir el panel. Revisa la primera línea roja de consola.</p>";
  ...
  function scheduleRightPanelChart(ctx, root) {
    ...
    const mount = (attempt = 0) => {
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 45 · Chart montado. Dashboard Ly preparado.";
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    const overlay = ctx.el("div", "s936-lyrics-modal-overlay");
    overlay.id = "s936-lyrics-modal-overlay";

    const modal = ctx.el("div", "s936-lyrics-modal");
    const head = ctx.el("div", "s936-lyrics-head");
    const title = ctx.el("div", "");
    title.appendChild(ctx.el("b", "", "Ly · Letra por compás"));
    title.appendChild(ctx.el("small", "", label + " · escribe una línea para karaoke / guía vocal"));
    ...
    for (let i = 0; i < bars; i++) {
      ...
      const lab = ctx.el("label", "", "Compás " + (i + 1));
      const ta = document.createElement("textarea");
      ta.placeholder = "Escribe la frase de este compás…";
    ...
    const save = ctx.el("button", "s936-lyrics-save", "✓ Guardar letra en la sección");
    const cancel = ctx.el("button", "s936-lyrics-cancel", "Cancelar");
    ...
    save.onclick = () => {
      ...
      try {
        window.dispatchEvent(new CustomEvent("studio936:section-lyrics-updated", {
          detail: { section, label, bars, lines: nextLines, source: "structure-lyrics-editor" }
        }));
      } catch(_) {}
      try {
        const panel = document.getElementById("s936-chart-view-panel");
        if (panel && window.Studio936SuiteProChart?.render) {
          window.Studio936SuiteProChart.render({ container: panel });
        }
      } catch(_) {}
      overlay.remove();
    };
  ...
  function renderZoomSessionConsole(ctx, root, s, part, parts) {
    ...
    function lyricToolButton() {
      ...
      btn.onclick = (e) => {
        ...
        setStatus("Editor Ly abierto: escribe una línea por compás para karaoke.");
      };
      return btn;
    }
  ...
  function renderBuilder(ctx, root, s, parts) {
    ...
    if (focusedPart) {
      // Cambio 45: se elimina el header grande "Zoom sección".
  ...
  function partRow(ctx, s, parts, part, index) {
    ...
    // Cambio 45: en Zoom sección, la barra del canal solo deja los controles principales.
