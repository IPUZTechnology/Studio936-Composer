// Studio 936 Composer - Suite Pro Structure / ADN Module v4.8.8 Cambio 93 (LIMPIEZA)
...
// Cambio 93 (LIMPIEZA — Fase 0 de la reestructuración): se consolidaron los
// 15 selectores CSS más repetidos del archivo (algunos definidos hasta 19
// veces en distintos Cambios históricos), resolviendo matemáticamente cuál
// era la regla que REALMENTE ganaba hoy (considerando !important y @media),
// y dejando una sola definición limpia por selector/contexto. Se verificó
// que las 15 reglas resuelven EXACTAMENTE IGUAL antes y después (script de
// verificación cruzada). Cero cambios de comportamiento visual esperados.
// De 6382 a 5762 líneas (620 líneas de redundancia eliminadas).
(function () {
  "use strict";

  // Cambio 63: versión visible en el propio panel — así se puede confirmar
  // de un vistazo, sin abrir DevTools, si el navegador está corriendo esta
  // versión o una anterior en caché. Bump este string en cada cambio futuro.
  const MODULE_CAMBIO_LABEL = "Cambio 93";

  const STYLE_ID = "s936SuiteProStructureStylesV488Cambio93";
...
/* ── ROW LAYOUT v4.2 ── */
#s936SuitePro .s936-ckpt-part-row{display:grid!important;align-items:center!important;gap:5px!important;padding:5px 6px!important;border:1px solid rgba(255,255,255,.09);border-radius:11px;background:rgba(255,255,255,.026)!important;transition:border-color .15s,background .15s;cursor:default;grid-template-columns:24px 58px minmax(0,1fr)!important;min-height:40px!important;border-color:rgba(255,255,255,.085)!important;max-width:100%!important;overflow-x:hidden!important;position:relative!important;justify-content:start!important;column-gap:6px!important;}
...
#s936SuitePro .s936-ckpt-part-num{color:#ffe066!important;font-size:.58rem!important;font-weight:950!important;min-width:20px!important;text-align:center;flex-shrink:0;opacity:1!important;grid-column:1!important;width:20px!important;height:24px!important;display:flex!important;align-items:center!important;justify-content:center!important;border-radius:8px!important;background:rgba(255,224,102,.09)!important;border:1px solid rgba(255,224,102,.18)!important;text-shadow:0 0 8px rgba(255,224,102,.18)!important;border-color:rgba(255,224,102,.24)!important;}
#s936SuitePro .s936-ckpt-part-badge{border-radius:7px!important;font-size:.48rem!important;font-weight:900;padding:3px 4px!important;text-transform:uppercase;letter-spacing:.5px;flex-shrink:0;white-space:nowrap!important;transform:none!important;transform-origin:left center;grid-column:2!important;grid-row:1!important;justify-self:stretch!important;order:0!important;opacity:.92!important;margin-left:0!important;align-self:center!important;text-align:center!important;max-width:58px!important;overflow:hidden!important;text-overflow:ellipsis!important;cursor:help!important;min-width:58px!important;}
#s936SuitePro .s936-ckpt-part-info{
  flex:1;
  min-width:0;
}
#s936SuitePro .s936-ckpt-part-name{font-size:.70rem!important;font-weight:900!important;color:#fff;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;line-height:1.05!important;letter-spacing:.05px!important;max-width:112px!important;}
...
#s936SuitePro .s936-ckpt-row-actions{display:flex!important;align-items:center!important;gap:4px!important;flex-shrink:0;padding-left:5px!important;border-left:1px solid rgba(255,224,102,.12)!important;grid-column:3!important;grid-auto-flow:column!important;grid-auto-columns:auto!important;justify-content:space-between!important;min-width:0!important;justify-self:stretch!important;width:100%!important;max-width:100%!important;margin-left:0!important;grid-template-columns:repeat(6,minmax(28px,1fr)) 17px 24px!important;flex-wrap:nowrap!important;}
...
#s936SuitePro .s936-ckpt-row-btn{width:auto!important;height:31px!important;border-radius:9px!important;border:1px solid rgba(232,210,138,.28)!important;background:linear-gradient(180deg,rgba(232,210,138,.065),rgba(232,210,138,.025))!important;color:#e8d28a!important;font-size:.62rem!important;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:border-color .15s,color .15s,background .15s;border-color:rgba(233,212,138,.23)!important;box-shadow:none!important;line-height:1!important;font-weight:950!important;min-width:25px!important;letter-spacing:.12px!important;flex:1 1 0!important;max-width:34px!important;}
#s936SuitePro .s936-ckpt-row-btn:hover{border-color:rgba(255,224,102,.65)!important;color:#ffe066!important;background:rgba(255,224,102,.105)!important;transform:translateY(-1px);box-shadow:0 0 0 1px rgba(255,224,102,.16) inset, 0 0 14px rgba(255,224,102,.10)!important;}
...
#s936SuitePro .s936-ckpt-row-action.zoom{border-color:rgba(0,255,204,.25);color:#ffe066!important;font-size:.74rem!important;width:33px!important;min-width:33px!important;height:33px!important;}
#s936SuitePro .s936-ckpt-row-action.lyric{border-color:rgba(255,91,234,.25);color:#ff9df4;font-size:.54rem!important;min-width:31px!important;width:31px!important;height:31px!important;}
#s936SuitePro .s936-ckpt-row-action.rec{border-color:rgba(255,80,80,.28);color:#ff9d9d;font-size:.50rem!important;letter-spacing:.15px!important;min-width:32px!important;width:32px!important;height:31px!important;}
...
.s936-lyrics-float-panel{position:fixed;z-index:99999;width:min(1180px,calc(100vw - 34px));height:min(620px,calc(100vh - 54px));left:max(18px,calc(50vw - 590px));top:96px;resize:both;overflow:hidden;display:flex;flex-direction:column;border:1px solid rgba(0,255,204,.26)!important;border-radius:20px!important;background:radial-gradient(circle at 10% 0%,rgba(0,255,204,.13),transparent 34%),
    radial-gradient(circle at 85% 8%,rgba(255,224,102,.10),transparent 30%),
    linear-gradient(145deg,rgba(5,13,15,.74),rgba(10,9,14,.62))!important;box-shadow:0 24px 80px rgba(0,0,0,.48),0 0 0 1px rgba(255,255,255,.07) inset,0 0 30px rgba(0,255,204,.055)!important;color:#fff;backdrop-filter:blur(20px) saturate(1.1)!important;border-color:rgba(0,255,204,.26)!important;/* Cambio 78:el panel ahora llena por defecto desde su posición (top:96px)
     hasta casi el final de la ventana, en vez de limitarse a 660px de alto —
     así se ven más compases sin necesidad de agrandar manualmente cada vez.
     Sigue siendo redimensionable (resize:both) si se quiere achicar. */
  height:calc(100vh - 112px);/* Cambio 68:se quita el  del ancho — con , ganaba
     siempre sobre el estilo inline que aplica el arrastre nativo del navegador,
     y por eso el panel solo crecía verticalmente (el alto sí podía cambiar
     porque no tenía ). Ahora resize:both funciona en las dos
     direcciones, igual que ya funcionaba el alto. */
  width:min(1180px,calc(100vw - 34px));/* Cambio 83:en vez de calcular un alto exacto por JS (que podía medirse
     antes de que el navegador terminara el layout, dejando un hueco vacío),
     el panel se autoajusta con height:auto + un tope máximo — el navegador
     mismo calcula la altura real del contenido, sin adivinar. SIN 
     en "height", igual que se corrigió con el ancho en el Cambio 68 — si no,
     le ganaría siempre al estilo inline que aplica el arrastre manual del
     redimensionado, y no se podría estirar a mano. */
  height:auto;max-height:calc(100vh - 112px);min-width:520px!important;min-height:300px!important;-webkit-backdrop-filter:blur(20px) saturate(1.1)!important;}
...
.s936-lyrics-float-panel .s936-lyrics-beat-cell textarea{width:100%;min-height:66px!important;max-height:none;resize:none;box-sizing:border-box;border:1px solid rgba(0,255,204,.14)!important;border-radius:10px!important;background:rgba(0,0,0,.22)!important;color:#fff;padding:8px 6px;font-size:.74rem!important;line-height:1.26!important;outline:none;font-weight:700!important;border-color:rgba(0,255,204,.13)!important;flex:1;text-align:center;}
...
@media(max-width:720px){
  .s936-lyrics-float-panel{left:10px!important;top:72px!important;width:calc(100vw - 20px)!important;height:calc(100vh - 92px)!important;}
  .s936-lyrics-float-panel .s936-lyrics-row{
...
/* Cambio 63 · versión visible del módulo, para confirmar de un vistazo si el
   navegador está corriendo lo último o una versión anterior en caché. */
.s936-lyrics-float-panel .s936-lyrics-version-badge{
  display:inline-flex;
  align-items:center;
  height:20px;
  padding:0 7px;
...
.s936-lyrics-float-panel .s936-lyrics-chart-grid{gap:10px!important;display:grid;grid-template-columns:repeat(4,minmax(185px,1fr))!important;align-items:stretch;min-width:880px;}
...
/* Cambio 73 (HOTFIX CRITICO): esta regla ocultaba con display:none TODA la
   barra de acciones (el boton "Guardar y ver en Chart" y "Cerrar") -- por eso
   el clic real del mouse nunca llegaba a activarlo. Resto de una version
   anterior del panel que reutilizaba el mismo nombre de clase. Eliminada. */
...
#s936SuitePro .s936-struct-card.s936-struct-arrangement-full{margin-top:0!important;padding-top:10px!important;border-color:rgba(0,255,204,.22)!important;background:radial-gradient(circle at 0% 0%,rgba(0,255,204,.07),transparent 30%),
    linear-gradient(180deg,rgba(255,255,255,.035),rgba(0,0,0,.10))!important;padding:8px 7px 10px!important;/* Cambio 91 (HOTFIX):esta tarjeta tenía su PROPIO scroll independiente
     (max-height basado en 100vh + overflow:auto), que se volvía realmente
     scrolleable en pantallas más chicas (el laptop) pero no en un monitor
     extendido más grande — de ahí la doble barra que solo aparecía en el
     laptop. Se quita: ahora esta tarjeta fluye normal, y solo el panel
     completo (una sola vez) se encarga de scrollear si hace falta. */
  scrollbar-width:thin;border-radius:16px!important;overflow-x:hidden!important;}
`;
    style.textContent += `
...
@media(max-height:760px){
  /* Cambio 91: se quita el max-height reforzado de esta tarjeta — ya no
     tiene scroll propio (ver arriba), así que no necesita un tope de
     altura específico para pantallas cortas. */
  #s936SuitePro .s936-ckpt-part-row{min-height:30px!important;padding-top:3px!important;padding-bottom:3px!important;}
  #s936SuitePro .s936-ckpt-row-action,
  #s936SuitePro .s936-ckpt-row-btn{width:19px!important;min-width:19px!important;height:19px!important;}
}
`;
    style.textContent += `
...
#s936SuitePro .s936-ckpt-row-gear .s936-ckpt-row-btn{width:23px!important;min-width:23px!important;height:31px!important;border-radius:9px!important;max-width:23px!important;}
@media(max-width:430px){
  #s936SuitePro .s936-ckpt-part-row{grid-template-columns:20px 54px minmax(0,1fr)!important;column-gap:4px!important;}
  #s936SuitePro .s936-ckpt-row-actions{grid-auto-columns:23px!important;gap:3px!important;grid-template-columns:repeat(6,minmax(25px,1fr)) 15px 22px!important;padding-left:4px!important;}
  #s936SuitePro .s936-ckpt-row-action,
  #s936SuitePro .s936-ckpt-row-btn{width:24px!important;min-width:24px!important;height:28px!important;}
}
`;
...
@media(max-width:430px){
  ...
  #s936SuitePro .s936-ckpt-row-action.play,
  #s936SuitePro .s936-ckpt-row-action.loop,
  #s936SuitePro .s936-ckpt-row-action.zoom{width:26px!important;min-width:26px!important;height:30px!important;}
}
...
#s936SuitePro:not(.is-max){
  ...
  /* Cambio 89: se quita "overflow-y:auto!important" de aquí — le ganaba a
     overflow:hidden que suite-pro.js le pone al panel exterior (Cambio 87),
     y eso causaba una segunda barra de scroll pegada a la del contenido
     interno (.s936-sp-content). Solo ese contenido interno debe scrollear. */
  /* Cambio 90: se agrega ":not(.is-max)" al selector — antes este ancho
     fijo (máximo ~560px) ganaba SIEMPRE, incluso con el modo MAX activo,
     impidiendo que el panel se expandiera de verdad al maximizar. */
...
@media(max-width:390px){
  #s936SuitePro .s936-ckpt-part-row{grid-template-columns:16px 52px auto!important;gap:3px!important;column-gap:3px!important;}
  #s936SuitePro .s936-ckpt-part-badge{max-width:52px!important;font-size:.43rem!important;}
  #s936SuitePro .s936-ckpt-row-actions{gap:3px!important;padding-left:3px!important;}
  #s936SuitePro .s936-ckpt-row-action.play,
  #s936SuitePro .s936-ckpt-row-action.loop,
  #s936SuitePro .s936-ckpt-row-action.zoom{width:29px!important;min-width:29px!important;height:30px!important;}
}
`;
...
@media(max-width:390px){
  ...
  #s936SuitePro .s936-ckpt-row-action,
  #s936SuitePro .s936-ckpt-row-btn{width:27px!important;min-width:27px!important;height:28px!important;}
  #s936SuitePro .s936-ckpt-row-action.play,
  #s936SuitePro .s936-ckpt-row-action.loop,
...
#s936SuitePro:not(.is-max){
  ...
  overflow-x:hidden!important;
  /* Cambio 89: ídem — se quita overflow-y:auto!important de aquí también
     (segunda ocurrencia, del bloque Cambio 44), por la misma razón. */
  /* Cambio 90: ídem — se agrega ":not(.is-max)" para que el ancho fijo no
     impida que el panel se expanda de verdad al maximizar. */
}
#s936SuitePro *{
  box-sizing:border-box!important;
}
...
@media(max-width:1280px){
  ...
  #s936SuitePro .s936-ckpt-part-row{grid-template-columns:22px 54px minmax(0,1fr)!important;gap:4px!important;}
  #s936SuitePro .s936-ckpt-part-badge{min-width:54px!important;max-width:54px!important;}
  #s936SuitePro .s936-ckpt-row-actions{gap:3px!important;padding-left:4px!important;}
  #s936SuitePro .s936-ckpt-row-action,
  #s936SuitePro .s936-ckpt-row-btn{min-width:22px!important;max-width:30px!important;height:29px!important;font-size:.56rem!important;}
...
@media (max-width:900px){
  .s936-lyrics-float-panel .s936-lyrics-chart-grid{grid-template-columns:repeat(2,minmax(190px,1fr));min-width:430px;}
}
...
/* Cambio 73 (HOTFIX CRITICO): esta regla ocultaba con display:none TODA la
   ...
   anterior del panel que reutilizaba el mismo nombre de clase. Eliminada. */
@media(max-width:900px){
  .s936-lyrics-float-panel{min-width:0!important;}
  .s936-lyrics-float-panel .s936-lyrics-chart-grid{grid-template-columns:repeat(2,minmax(185px,1fr))!important;}
}

/* Cambio 51 · ícono de nota para duración (reemplaza el <select> de texto) + texto de letra más legible */

/* Cambio 58: el badge de nota deja de superponerse dentro del cuadro de texto
   (eso confundía al escribir) — ahora es un elemento propio, pequeño, DEBAJO
   del cuadro, sin taparlo. El nombre de la nota sigue siendo la guía dentro
   del placeholder (Cambio 55). */
...
/* Cambio 59 · mini-pentagrama de vista previa: dibuja la(s) nota(s) grabadas
   (una sola, o el tresillo/cuatrillo completo) directamente sobre la celda,
   visibles aunque el popover esté cerrado. */
.s936-lyric-mini-staff{
  position:relative;
  width:44px;
  height:22px;
  flex:0 0 auto;
...
.s936-lyric-mini-staff.is-empty{
  opacity:.32;
}
.s936-lyric-mini-staff-lines{
  position:absolute;
  inset:0;
}
.s936-lyric-mini-staff-line{
  position:absolute;
  left:0;
  right:0;
  height:1px;
  background:rgba(0,255,204,.30);
}
.s936-lyric-mini-staff-notes{
  position:absolute;
  inset:0;
}
.s936-lyric-mini-note{
  position:absolute;
  width:6px;
  height:6px;
  margin-left:-3px;
  ...
  border-radius:50%;
  background:#ffe066;
  box-shadow:0 0 5px rgba(255,224,102,.6);
}
...
/* Cambio 58: mientras un popover de nota está abierto, se le abre el overflow
   a toda la cadena de contenedores (panel, cuerpo, grid, tarjeta) para que el
   pentagrama nunca quede recortado — mucho más simple y confiable que mover
   el nodo a <body> con position:fixed. */
.s936-lyrics-float-panel.s936-lyrics-popover-open{
  overflow:visible!important;
}
.s936-lyrics-float-panel.s936-lyrics-popover-open .s936-lyrics-body{
  overflow:visible!important;
}
.s936-lyrics-float-panel.s936-lyrics-popover-open .s936-lyrics-chart-grid,
.s936-lyrics-float-panel.s936-lyrics-popover-open .s936-lyrics-bar-card,
.s936-lyrics-float-panel.s936-lyrics-popover-open .s936-lyrics-beat-cell{
  overflow:visible!important;
}
...
/* Cambio 66 · nota seleccionada, lista para moverse con el siguiente clic. */
.s936-note-row.is-selected{
  background:rgba(255,224,102,.22)!important;
}
.s936-note-row.is-selected .s936-note-seq-dot{
  animation:s936NoteSeqPulse 0.9s ease-in-out infinite;
}
@keyframes s936NoteSeqPulse{
  0%,100%{ box-shadow:0 0 0 2px rgba(4,7,10,.9),0 0 6px rgba(255,224,102,.5); }
  50%{ box-shadow:0 0 0 3px rgba(4,7,10,.9),0 0 12px rgba(255,224,102,.95); }
}
...
/* Cambio 64 · marca, con un puntito numerado, cada nota ya puesta cuando el
   tiempo tiene más de una (tresillo/cuatrillo) — todas visibles a la vez
   sobre el mismo pentagrama, en el orden en que se hizo clic. */
.s936-note-row .s936-note-seq-dot{
  position:absolute;
  top:50%;
  transform:translate(-50%,-50%);
  min-width:14px;
  ...
  line-height:14px;
  text-align:center;
  box-shadow:0 0 0 2px rgba(4,7,10,.9),0 0 6px rgba(255,224,102,.5);
  pointer-events:none;
}
.s936-note-staff-holder{
  display:flex;
  justify-content:center;
}
...
`;
  ...
  // Cambio 61: los glyphs ♩𝅗𝅥𝅗𝅥.𝅝 usan caracteres Unicode del plano suplementario
  // (Musical Symbols) que muchos sistemas Windows no tienen instalados en
  // ninguna fuente — por eso se veían como una "X" o cuadro roto. Se
  // reemplazan por SVG dibujados a mano, que se ven igual en cualquier equipo.
  function svgNoteIcon(kind) {
    const filled = kind === "q";
    const hasStem = kind !== "w";
    const hasDot = kind === "hd";
    return (
      '<svg viewBox="0 0 16 16" width="13" height="13" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
      '<ellipse cx="5.5" cy="11.3" rx="3.1" ry="2.2" transform="rotate(-18 5.5 11.3)" ' +
      'fill="' + (filled ? "currentColor" : "none") + '" stroke="currentColor" stroke-width="1.3"/>' +
      (hasStem ? '<line x1="8.4" y1="10.9" x2="8.4" y2="1.4" stroke="currentColor" stroke-width="1.3"/>' : "") +
      (hasDot ? '<circle cx="12.1" cy="11.3" r="1" fill="currentColor"/>' : "") +
      "</svg>"
    );
  }
  const LYRIC_NOTE_ICON = { 1: svgNoteIcon("q"), 2: svgNoteIcon("h"), 3: svgNoteIcon("hd"), 4: svgNoteIcon("w") };
  // Cambio 61: versión en texto plano SOLO para el placeholder del cuadro de
  // texto (los placeholders no pueden contener HTML/SVG).
  const LYRIC_NOTE_TEXT = { 1: "1T", 2: "2T", 3: "3T", 4: "4T" };
  ...
  // Cambio 64: pentagrama único y compartido — soporta de 1 a 4 notas por
  // tiempo con clic secuencial. Cada clic llena la siguiente nota vacía;
  // todas las notas ya puestas quedan marcadas juntas en el mismo plano.
  // Reemplaza el picker de una sola nota y los pickers separados por sílaba.
  function buildSequentialStaffPicker(ctx, count, initialPitches, onChange) {
    const wrap = ctx.el("div", "s936-note-staff-wrap");
    const label = ctx.el("div", "s936-note-pitch-label", "");
    const staff = ctx.el("div", "s936-note-staff");

    let pitches = (initialPitches || []).slice(0, count);
    while (pitches.length < count) pitches.push("");
    // Cambio 66: qué nota está "seleccionada" para moverse (null = ninguna).
    ...
    NOTE_STAFF_STEPS.forEach((step) => {
      ...
      row.onclick = (e) => {
        e.stopPropagation();
        // Cambio 66: 1) si hay una nota seleccionada, este clic la MUEVE aquí.
        if (selectedIdx !== null) {
          pitches[selectedIdx] = step.pitch;
          selectedIdx = null;
          renderDots();
          onChange(pitches.slice());
          return;
        }
        // 2) si clico sobre una nota YA puesta (en cualquier altura), la
        // selecciono en vez de agregar una nueva — así puedo moverla con el
        // siguiente clic, sin importar si es la 1ra, 2da, 3ra o 4ta.
        const usedIdx = pitches.indexOf(step.pitch);
        if (usedIdx !== -1) {
          selectedIdx = usedIdx;
          renderDots();
          return;
        }
        // 3) si no hay selección y esta altura está libre, llena la
        // siguiente nota vacía (comportamiento original para ir poniendo).
        const emptyIdx = pitches.findIndex((p) => !p);
        if (emptyIdx === -1) return; // ya están las 4 puestas; hay que seleccionar una para moverla.
        pitches[emptyIdx] = step.pitch;
        renderDots();
        onChange(pitches.slice());
      };
      staff.appendChild(row);
      rowByPitch[step.pitch] = row;
    });

    function renderDots() {
      Object.values(rowByPitch).forEach((row) => {
        row.classList.remove("is-active", "is-selected");
        row.querySelectorAll(".s936-note-seq-dot").forEach((d) => d.remove());
      });
      pitches.forEach((p, idx) => {
        if (!p || !rowByPitch[p]) return;
        const row = rowByPitch[p];
        row.classList.add("is-active");
        if (idx === selectedIdx) row.classList.add("is-selected");
        const dot = ctx.el("span", "s936-note-seq-dot", count > 1 ? String(idx + 1) : "");
        // Cambio 65: cada nota se ubica de izquierda a derecha según el orden
        // en que se puso (la 1ra más a la izquierda, la última más a la
        // derecha), como en un pentagrama real — no todas apiladas al centro
        // como tablatura.
        const leftPct = ((idx + 1) / (count + 1)) * 100;
        dot.style.left = leftPct + "%";
        row.appendChild(dot);
        row.title = noteSolfa(p) + (count > 1 ? " · nota " + (idx + 1) + " — clic para seleccionarla y moverla" : " — clic en otra altura para moverla");
      });
      // Cambio 65: el título siempre muestra los nombres de las notas ya
      // puestas (sean 1 o 4), en vez de un mensaje de estado genérico.
      // Cambio 66: si hay una seleccionada para mover, lo indica claramente.
      const placedNames = pitches.filter(Boolean).map(noteSolfa);
      if (selectedIdx !== null) {
        label.textContent = "Nota " + (selectedIdx + 1) + " seleccionada — clic para moverla";
      } else if (!placedNames.length) {
        label.textContent = count > 1 ? ("Nota 1 de " + count) : "Sin nota";
      } else {
        label.textContent = placedNames.join(" - ");
      }
    }
    renderDots();
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    const subdivisions = current.subdivisions || {}; // Cambio 56: cuántas notas por tiempo
    const subPitches = current.subPitches || {};     // Cambio 56: altura de cada nota
    ...
    const versionBadge = ctx.el("span", "s936-lyrics-version-badge", MODULE_CAMBIO_LABEL);
    versionBadge.title = "Versión del módulo Ly Letra actualmente cargada en tu navegador. Si no coincide con lo último que te entregué, hace falta desplegar/recargar.";
    ...
    title.append(titleText, versionBadge, infoTip, titleHint);
    ...
    const saveTop = lyricTool("💾", "Guardar y reflejar en el Chart");
    const draftTop = lyricTool("📝", "Guardar borrador (también se refleja en el Chart)");
    const boardTop = lyricTool("⇢", "Guardar y reflejar en el Chart");
    ...
    // Cambio 58: el "portal" a <body> con position:fixed del Cambio 55 resultó
    // frágil (el panel usa backdrop-filter, que en algunos navegadores puede
    // alterar el sistema de coordenadas de los descendientes position:fixed,
    // y el popover terminaba apareciendo lejos del ícono, cerca del dock).
    // Nuevo enfoque, más simple y confiable: el popover se queda anclado con
    // position:absolute a su propia celda (como siempre), y en vez de moverlo
    // de lugar, se le "abre" temporalmente el overflow al panel mientras hay
    // un popover abierto, así nunca se recorta.
    const openLyricPopovers = new Set();
    function openLyricPopover(pop) {
      panel.classList.add("s936-lyrics-popover-open");
      pop.hidden = false;
    ...
    function closeLyricPopover(pop) {
      ...
      if (!openLyricPopovers.size) panel.classList.remove("s936-lyrics-popover-open");
    }
    function closeAllLyricPopovers() {
      openLyricPopovers.forEach((pop) => { pop.hidden = true; });
      openLyricPopovers.clear();
      panel.classList.remove("s936-lyrics-popover-open");
    }
    ...
    for (let i = 0; i < bars; i++) {
      ...
      for (let b = 0; b < 4; b++) {
        ...
        // Cambio 51/53/64: ícono de nota; clic abre EL pentagrama compartido
        // (altura, 1-4 notas por clic secuencial) + 4 figuras (duración de
        // sostenido). El cuadro de texto de la letra (ta) NUNCA se oculta ni
        // se reemplaza — la subdivisión solo afecta cuántas notas se marcan
        // en el pentagrama, no la escritura de la letra.
        ...
        const legacyPitch = String(barPitches[String(b)] || barPitches[b] || "").trim();
        const storedSubPitches = Array.isArray(barSubPitches[String(b)] || barSubPitches[b]) ? (barSubPitches[String(b)] || barSubPitches[b]) : [];
        let initialPitchesArr = storedSubPitches.length ? storedSubPitches.slice(0, initialSubdivision) : (legacyPitch ? [legacyPitch] : []);
        while (initialPitchesArr.length < initialSubdivision) initialPitchesArr.push("");
        ...
        durationWrap.dataset.subdivision = String(initialSubdivision);
        durationWrap.dataset.pitches = JSON.stringify(initialPitchesArr);

        const durationBtn = ctx.el("button", "s936-lyric-duration-btn", "");
        durationBtn.innerHTML = LYRIC_NOTE_ICON[initialDur];
        durationBtn.type = "button";
        const btnLabel = () => {
          const arr = JSON.parse(durationWrap.dataset.pitches || "[]").filter(Boolean);
          return LYRIC_NOTE_LABEL[Number(durationWrap.dataset.value)] + (arr.length ? " · " + arr.map(noteSolfa).join("-") : "");
        };
        durationBtn.title = btnLabel();
        durationBtn.setAttribute("aria-label", "Nota: " + btnLabel());

        // Cambio 67: el cuadro de texto solo debe mostrar lo que el usuario
        // escriba — la nota vive en el pentagrama/ícono, no aquí.
        ta.placeholder = "letra";

        const durationPop = ctx.el("div", "s936-lyric-duration-pop");
        durationPop.hidden = true;

        const staffHolder = ctx.el("div", "s936-note-staff-holder");
        durationPop.appendChild(staffHolder);

        // Cambio 64: reconstruye el pentagrama según cuántas notas tenga
        // activas este tiempo (1-4). Clic secuencial: cada clic llena la
        // siguiente nota vacía y todas quedan marcadas juntas ahí mismo.
        function rebuildStaff() {
          staffHolder.innerHTML = "";
          const n = Number(durationWrap.dataset.subdivision) || 1;
          let arr = JSON.parse(durationWrap.dataset.pitches || "[]");
          arr = arr.slice(0, n);
          while (arr.length < n) arr.push("");
          const picker = buildSequentialStaffPicker(ctx, n, arr, (nextArr) => {
            durationWrap.dataset.pitches = JSON.stringify(nextArr);
            durationBtn.title = btnLabel();
            durationBtn.setAttribute("aria-label", "Nota: " + btnLabel());
          });
          staffHolder.appendChild(picker);
        }
        rebuildStaff();

        const durationOptsRow = ctx.el("div", "s936-lyric-duration-pop-durations");
        [1, 2, 3, 4].forEach((n) => {
          const opt = ctx.el("button", "s936-lyric-duration-opt", "");
          opt.innerHTML = LYRIC_NOTE_ICON[n];
          opt.type = "button";
          ...
          opt.onclick = (e) => {
            ...
            durationBtn.innerHTML = LYRIC_NOTE_ICON[n];
            durationBtn.title = btnLabel();
            durationBtn.setAttribute("aria-label", "Nota: " + btnLabel());
        ...
        // Cambio 64: "cuántas notas tiene este tiempo" (antes "dividir en
        // sílabas") — ahora solo controla el pentagrama, no la escritura.
        const subdivideRow = ctx.el("div", "s936-lyric-subdivide-row");
        subdivideRow.appendChild(ctx.el("span", "s936-lyric-subdivide-label", "Notas de tiempo"));
        const subdivideBtns = ctx.el("div", "s936-lyric-subdivide-btns");
        [1, 2, 3, 4].forEach((n) => {
          const sb = ctx.el("button", "s936-lyric-subdivide-opt", String(n));
          sb.type = "button";
          sb.title = n === 1 ? "Una sola nota" : (n === 3 ? "Tresillo (3 notas)" : n === 4 ? "Cuatrillo (4 notas)" : "2 notas");
          ...
          sb.onclick = (e) => {
            ...
            durationWrap.dataset.subdivision = String(n);
            rebuildStaff();
          };
        ...
        subdivisionFields.push({
          bar: String(i),
          beat: String(b),
          getSubdivision: () => Number(durationWrap.dataset.subdivision) || 1,
          getPitches: () => {
            try { return JSON.parse(durationWrap.dataset.pitches || "[]"); } catch (_) { return []; }
          }
        });

        // Cambio 55/58: el popover se anida con position:absolute en su
        // propia celda; el panel "abre" su overflow mientras está visible
        // para que nunca quede recortado.
        durationBtn.onclick = (e) => {
          e.stopPropagation();
          const wasHidden = durationPop.hidden;
          closeAllLyricPopovers();
          if (wasHidden) openLyricPopover(durationPop);
        };

        durationWrap.append(durationBtn, durationPop);
        durationFields.push(durationWrap);

        inputWrap.append(ta, durationWrap);
    ...
    function saveLyrics(closeAfterSave) {
      ...
      // Cambio 64: 1 a 4 notas por tiempo, elegidas con clic secuencial sobre
      // UN solo pentagrama. Se guardan como array (incluso si es una sola),
      // más "pitches" simple (primera nota) para compatibilidad hacia atrás.
      subdivisionFields.forEach((sf) => {
        const n = sf.getSubdivision();
        const pitchesArr = sf.getPitches().slice(0, n).map((p) => String(p || "").trim());
        const hasAny = pitchesArr.some(Boolean);
        const hasText = String(nextBeats?.[sf.bar]?.[sf.beat] || "").trim();
        if (!hasAny || !hasText) return;
        ...
        if (n > 1) {
          ...
          nextSubPitches[sf.bar][sf.beat] = pitchesArr;
        }
        if (pitchesArr[0]) {
          if (!nextPitches[sf.bar]) nextPitches[sf.bar] = {};
          nextPitches[sf.bar][sf.beat] = pitchesArr[0];
        }
      });
      ...
      const savedOk = saveSectionLyricsStore(data);
      ...
      try {
        window.dispatchEvent(new CustomEvent("studio936:section-lyrics-updated", {
          detail: {
            ...
            subdivisions: nextSubdivisions, subPitches: nextSubPitches,
            source: "structure-lyrics-floating-editor"
          }
        }));
      } catch(_) {}

      // Cambio 70: este bloque quedaba en silencio si no encontraba el panel
      // del Chart (por ejemplo, si aún no había terminado de montarse) — no
      // había ningún aviso, así que un fallo se veía igual que un éxito.
      // Ahora se reintenta una vez y, si de verdad falla, se avisa.
      ...
      let chartRefreshed = false;
      try { chartRefreshed = refreshChart(); } catch(_) { chartRefreshed = false; }
      if (!chartRefreshed) {
        console.warn("[Ly Letra] No se encontró #s936-chart-view-panel (o Studio936SuiteProChart.render) al guardar. Reintentando en 200ms...");
        setTimeout(() => {
          try {
            if (refreshChart()) return;
          } catch(_) {}
          console.warn("[Ly Letra] El Chart sigue sin poder actualizarse tras el reintento.");
          toast(ctx, "Se guardó, pero no se pudo actualizar el Chart automáticamente. Cierra y vuelve a abrir Ly Letra, o recarga la página.");
        }, 200);
      }
      if (!savedOk) {
        toast(ctx, "No se pudo guardar (el navegador bloqueó el almacenamiento local). Revisa el modo incógnito o los permisos del sitio.");
      }
    ...
    draftTop.onclick = (e) => {
      e.stopPropagation();
      saveLyrics(false);
      toast(ctx, "Borrador guardado y reflejado en el Chart.");
    };
    ...
    // Cambio 75: los íconos de guardar viven arriba (💾 y ⇢); se quitó la
    // barra de abajo con "✓ Guardar y ver en Chart" y "Cerrar" (redundante —
    // el cierre ya lo cubre el ✕ de la barra superior).
    saveTop.onclick = (e) => {
      e.stopPropagation();
      saveLyrics(false);
      saveTop.classList.add("saved");
      setTimeout(() => saveTop.classList.remove("saved"), 850);
      toast(ctx, "Guardado — reflejado en el Chart.");
    };

    boardTop.onclick = (e) => {
      e.stopPropagation();
      saveLyrics(false);
      toast(ctx, "Guardado y reflejado en el Chart.");
    };

    // Cambio 51: cerrar el popover de duración abierto si se hace clic en otra parte del panel.
    ...
    // Cambio 79: se corrige el alto (ver nota arriba en el historial).
    // Cambio 80: el ancho ya no es un tope fijo (1180px) ni el ancho total de
    // la ventana — va desde el borde del dock hasta el borde derecho REAL del
    // Chart (el contenedor de los compases), para no quedar ni más angosto
    // ni más ancho que la partitura visible.
    try {
      ...
      const rightEdge = (chartRect && chartRect.right > left + 300) ? chartRect.right : (window.innerWidth - 24);
      const width = Math.max(520, rightEdge - left - 6);
      panel.style.left = left + "px";
      panel.style.width = width + "px";
      const minTop = 96;
      panel.style.top = minTop + "px";
      // Cambio 83: en vez de calcular la altura a mano (podía medirse antes
      // de que el navegador terminara el layout, dejando un hueco vacío), el
      // CSS (height:auto + max-height) ya hace que el panel se autoajuste a
      // su contenido real. Solo falta, una vez que el navegador terminó de
      // pintarlo, leer su tamaño YA renderizado y anclarlo al fondo de la
      // ventana — así los primeros compases del Chart quedan visibles arriba.
      requestAnimationFrame(() => {
        try {
          const rendered = panel.getBoundingClientRect();
          const bottomMargin = 16;
          const top = Math.max(minTop, window.innerHeight - rendered.height - bottomMargin);
    ...
    // Cambio 79 (HOTFIX): antes solo se excluía el botón "close" (✕) del
    // inicio de arrastre — cualquier OTRO botón del header (💾, 📝, ⇢, 🎙, ✎)
    // disparaba pointerdown → preventDefault() + captura del puntero, lo que
    // impedía que el clic real llegara nunca al botón. Por eso "no funcionaban".
    // Ahora se excluye cualquier botón del toolbar (clase s936-lyrics-tool).
    let drag = null;
    head.addEventListener("pointerdown", (event) => {
      if (event.target.closest?.(".s936-lyrics-tool")) return;
      drag = {
  ...
  function renderZoomSessionConsole(ctx, root, s, part, parts) {
    ...
    function midiToolButton() {
      const btn = ctx.el("button", "s936-zoom-session-btn tool");
      btn.innerHTML = `<strong>🎹 MIDI / Teclado</strong><small>controlador</small>`;
      btn.onclick = (e) => {
        e.stopPropagation();
        // Cambio 86: ya no es decorativo — abre de verdad el módulo real de
        // MIDI IN que vive en Studio (js/suite-pro-midi.js), donde se puede
        // conectar y usar un teclado MIDI real.
        try { window.Studio936SuitePro?.openStudioTool?.("midi"); } catch(_) {}
        setStatus("Abriendo MIDI IN en Studio — conecta tu teclado ahí.");
      };
      return btn;
    }
