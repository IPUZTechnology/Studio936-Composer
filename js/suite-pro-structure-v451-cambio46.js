// Studio 936 Composer - Suite Pro Structure / ADN Module v4.5.1 Cambio 46
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV451Cambio46";
  ...
  function installDockFlexGuard() {
    ...
    const sync = () => {
      try {
        ...
        if (chartPanel) {
          // Cambio 46: no empujar el Chart con margin-left; eso generaba scroll horizontal
...
/* Cambio 46 · Lyric editor inline: no overlay negro, no invade el Chart */
...
`;
    ...
    style.textContent += `
/* Cambio 46 · Dashboard visual + editor Ly por compás */
#s936SuitePro .s936-zoom-session-console{
...
`;
  ...
  function scheduleRightPanelChart(ctx, root) {
    ...
    const mount = (attempt = 0) => {
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 46 · Chart montado. Dashboard Ly preparado.";
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    // Cambio 46: el editor Ly ya no abre overlay negro global.
    // Vive dentro de la consola ampliada de Zoom sección para no tapar el Chart.
    ...
    const lines = current.lines || current.bars || current || {};

    const host =
      document.querySelector("#s936SuitePro .s936-zoom-session-console") ||
      document.getElementById("s936SuitePro") ||
      document.body;

    const panel = ctx.el("section", "s936-lyrics-inline-panel");
    panel.id = "s936-lyrics-inline-panel";

    const head = ctx.el("div", "s936-lyrics-head");
    const title = ctx.el("div", "");
    title.appendChild(ctx.el("b", "", "Ly · Letra por compás"));
    title.appendChild(ctx.el("small", "", label + " · escribe una línea por compás; se verá debajo del Chart."));
    const close = ctx.el("button", "s936-lyrics-close", "×");
    close.title = "Cerrar editor de letra";
    ...
    for (let i = 0; i < bars; i++) {
      const row = ctx.el("div", "s936-lyrics-row");
      const lab = ctx.el("label", "", "C " + (i + 1));
      const ta = document.createElement("textarea");
      ta.placeholder = "Letra del compás " + (i + 1);
      ta.value = String(lines[String(i)] || lines[i] || "");
    ...
    function saveLyrics(closeAfterSave) {
      ...
      fields.forEach((field) => {
        ...
        if (value) nextLines[field.dataset.bar] = value;
      });
      ...
      try {
        window.dispatchEvent(new CustomEvent("studio936:section-lyrics-updated", {
          detail: { section, label, bars, lines: nextLines, source: "structure-lyrics-inline-editor" }
        }));
      } catch(_) {}
      ...
      setTimeout(() => { save.textContent = "✓ Guardar y ver en Chart"; }, 900);
      if (closeAfterSave) panel.remove();
    }
    ...
    const before =
      host.querySelector(".s936-zoom-session-block") ||
      host.querySelector(".s936-zoom-session-send") ||
      host.querySelector(".s936-zoom-session-status");
  ...
  function renderZoomSessionConsole(ctx, root, s, part, parts) {
    ...
    function lyricToolButton() {
      ...
      btn.onclick = (e) => {
        ...
        setStatus("Editor Ly abierto dentro de la consola: escribe una línea por compás y guárdala en el Chart.");
      };
      return btn;
    }
  ...
  function renderBuilder(ctx, root, s, parts) {
    ...
    if (focusedPart) {
      // Cambio 46: se elimina el header grande "Zoom sección".
  ...
  function partRow(ctx, s, parts, part, index) {
    ...
    // Cambio 46: en Zoom sección, la barra del canal solo deja los controles principales.
