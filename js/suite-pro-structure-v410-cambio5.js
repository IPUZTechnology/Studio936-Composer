// Studio 936 Composer - Suite Pro Structure / ADN Module v4.1.0 Cambio 5
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV410Cambio5";
  ...
  function register() {
    ...
    window.Studio936SuiteProStructure = { version: "structure-v4.1.0-cambio-5", render };
...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      console.error("Suite Pro Structure Cambio 5 render error:", error);
      const err = ctx.el("section", "s936-struct-card dangerzone");
      err.innerHTML = "<h4>Cambio número 5 · Error visible en Estructura</h4><p>No se pudo construir el panel. Revisa la primera línea roja de consola.</p>";
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 5 · Tracker activo · Chart al panel derecho");
    root.appendChild(banner);
  }

  function scheduleRightPanelChart(ctx, root) {
    const status = ctx.el("section", "s936-struct-right-status");
    status.textContent = "Cambio número 5 · Estructura abrió. Enviando Chart al panel grande derecho…";
    root.appendChild(status);

    const ensureChartScriptLoaded = () => {
      const src = "js/suite-pro-chart-v204-cambio5.js";
      const already = Array.from(document.scripts || []).some((script) => String(script.src || "").includes(src));
      if (already) return;
      const script = document.createElement("script");
      script.src = src + "?v=cambio-5-20260628";
      script.dataset.s936Cambio5ChartLoader = "true";
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        status.textContent = "Cambio número 5 · Cargando Chart para panel derecho… intento " + (attempt + 1) + ".";
        if (attempt === 0) ensureChartScriptLoaded();
        if (attempt < 12) {
          setTimeout(() => mount(attempt + 1), 160 + attempt * 120);
        } else {
          status.textContent = "Cambio número 5 · No encontré Studio936SuiteProChart.mountInRightPanel(). Revisa que index.html cargue suite-pro-chart-v204-cambio5.js.";
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 5 · Chart montado en el panel derecho. Selector: Piano · Guitarra · Ukulele · Bajo.";
          status.style.color = "#bfffee";
        } else {
          status.textContent = "Cambio número 5 · Chart no pudo montarse en el panel derecho: " + (result?.reason || "sin razón reportada") + ".";
          status.style.color = "#ffb5b5";
        }
      } catch (error) {
        console.warn("Suite Pro Structure Cambio 5 Chart right panel:", error);
        status.textContent = "Cambio número 5 · Error al montar Chart en panel derecho. Revisa la primera línea roja de consola.";
        status.style.color = "#ffb5b5";
      }
    };
