// Studio 936 Composer - Chart View v2.1.3 Cambio 14 (BRIDGE DE PRÁCTICA CON MAIN + REPETICIÓN EXPANDIDA)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.1.3-cambio-14";
  const STYLE_ID = "s936-chart-v213-cambio14";
  ...
  function startChartCentralPracticeBridge({ withPulse = false } = {}) {
    ...
    // Bridge seguro: si el Main ya expone groove/ritmo, se usa.
    // Si todavía no existe, el Chart conserva su reloj interno como fallback.
    started = tryBridgeCall(
      ["startGroove", "startRhythm", "startPracticeGroove", "startChartPractice"],
      { bpm, style, withPulse, source: "Studio936SuiteProChart", version: VERSION }
    );

    if (withPulse) {
      tryBridgeCall(["setMetronome", "setClick", "enableClick"], true);
    }
  ...
  function startChartRhythmConsole(container, { withPulse = false } = {}) {
    ...
    if (sourceEl) {
      const isMain = bridgeState.source === "main";
      sourceEl.textContent = isMain
        ? "Motor: Main/Groove · BPM " + bridgeState.bpm
        : "Motor: Interno fallback · BPM " + bridgeState.bpm;
    ...
    const runStep = () => {
      ...
      if (_chartRhythmPulse && bridgeState.source !== "main") {
      ...
      if (mode === "hit" || mode === "repeat" || mode === "hold") {
        const midis = chordToChartMidis(step.chord);
        if (midis.length) {
          // Mientras no exista un sink armónico real en el Main, el Chart mantiene
          // audio de referencia para practicar. Cuando el bridge reciba el acorde,
          // el Main podrá reemplazar este fallback sin cambiar el Chart.
          schedulePopupRhythmHit(midis, bridgeState.bpm || getCurrentChartBpm(), false, mode);
        }
      }
...
.s936-ch-dblbar{height:3px;margin-top:2px;background:linear-gradient(to right,rgba(255,255,255,.15) 0,rgba(255,255,255,.15) calc(100% - 4px),rgba(255,255,255,.5) calc(100% - 4px),rgba(255,255,255,.5) 100%)}
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 14 · Main manda pulso · Chart manda armonía";
    ...
    consoleTitle.textContent = "🎛 Mini consola práctica";
