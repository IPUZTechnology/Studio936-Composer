// Studio 936 Composer - Suite Pro Structure / ADN Module v4.3.8 Cambio 34
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV438Cambio33";
...
/* Cambio 34 · Limpieza del dock: no trackers/banners temporales */
#s936SuitePro .s936-struct-change-banner,
#s936SuitePro .s936-struct-right-status,
#s936SuitePro .s936-ckpt-topbar,
...
/* Cambio 34 · Dock como consola por sección: menú arriba, botones sobrios y agrupados */
#s936SuitePro .s936-ckpt-topbar,
#s936SuitePro .s936-ckpt-topbar-compact,
#s936SuitePro .s936-ckpt-menu-btn-wide,
#s936SuitePro .s936-ckpt-dropdown{
...
`;
  ...
  function render(ctx, shell) {
    ...
    // Cambio 34: sin banners temporales en el dock; el menú de canción vive en Compose.
    ...
    try {
      ...
      // Cambio 34: el menú de canción sube al nivel Compose; no duplicar cabecera en Estructura.
      ...
      err.innerHTML = "<h4>Cambio número 34 · Error visible en Estructura</h4><p>No se pudo construir el panel. Revisa la primera línea roja de consola.</p>";
  ...
  function scheduleRightPanelChart(ctx, root) {
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        status.textContent = "Cambio número 34 · Cargando Chart… intento " + (attempt + 1) + ".";
        if (attempt === 0) ensureChartScriptLoaded();
        if (attempt < 12) {
          setTimeout(() => mount(attempt + 1), 160 + attempt * 120);
        } else {
          status.textContent = "Cambio número 34 · No encontré Studio936SuiteProChart.mountInRightPanel(). Revisa que index.html cargue suite-pro-chart-v232-cambio32.js.";
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 34 · Chart montado. Panel izquierdo = consola por sección limpia.";
