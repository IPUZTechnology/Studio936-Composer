// Studio 936 Composer - Suite Pro Structure / ADN Module v4.2.6 Cambio 21
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV426Cambio21";
  ...
  function register() {
    ...
    window.Studio936SuiteProStructure = { version: "structure-v4.2.5-cambio-21", render };
...
/* Cambio 21 · Navegación musical visible antes del arreglo */
...
/* Cambio 21 · Mini consola sesión real; las marcas se editan en el Chart */
...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      // Cambio 21: las marcaciones se editan directamente en el Chart; el panel izquierdo queda para consola.
      renderSessionConsole(ctx, root, s, parts);
      renderBuilder(ctx, root, s, parts);
      scheduleRightPanelChart(ctx, root);
    } catch (error) {
      console.error("Suite Pro Structure Cambio 21 render error:", error);
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 21 · Mini consola sesión + Chart limpio");
  ...
  function renderNavigationPanel(ctx, root, s, parts) {
    ...
    card.appendChild(ctx.el("p", "s936-struct-muted", "Define las señales del mapa profesional: repeticiones, Fine, Segno, Coda y casillas. Cambio 21 las deja visibles arriba y dibujadas claramente en el Chart; la obediencia completa en práctica viene después."));
  ...
  function renderSessionConsole(ctx, root, s, parts) {
    ...
    titleWrap.appendChild(ctx.el("small", "", "Panel de práctica · el Chart es la partitura"));
    ...
    loop.onclick = () => {
      ...
      setEngine("Loop sección preparado", true);
    };
    pulse.onclick = () => {
      const panel = chartPanel();
      const ok = window.Studio936SuiteProChart?.startChartRhythmConsole?.(panel, { withPulse: true });
    ...
    const note = ctx.el("div", "s936-session-console-note", "Play superior = canción completa. Mini consola = práctica de la parte/sección. Las marcas de partitura se ponen en el encabezado del compás del Chart.");
