// Studio 936 Composer - Suite Pro Structure / ADN Module v4.4.1 Cambio 36
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV440Cambio35";
  ...
  function register() {
    ...
    window.Studio936SuiteProStructure = { version: "structure-v4.3.7-cambio-32", render };
...
/* Cambio 36 · Limpieza del dock: no trackers/banners temporales */
#s936SuitePro .s936-struct-change-banner,
#s936SuitePro .s936-struct-right-status,
#s936SuitePro .s936-ckpt-topbar,
...
/* Cambio 36 · Dock como consola por sección: menú arriba, botones sobrios y agrupados */
#s936SuitePro .s936-ckpt-topbar,
#s936SuitePro .s936-ckpt-topbar-compact,
#s936SuitePro .s936-ckpt-menu-btn-wide,
#s936SuitePro .s936-ckpt-dropdown{
...
`;
    style.textContent += `
/* Cambio 36 · Panel izquierdo autoajustado y más compacto.
...
`;
    style.textContent += `
/* Cambio 36 · Consola por sección más clara: nombre primero, botones definidos y flechas compactas */
#s936SuitePro .s936-struct-card.s936-struct-arrangement-full{
...
`;
  ...
  function render(ctx, shell) {
    ...
    // Cambio 36: sin banners temporales en el dock; el menú de canción vive en Compose.
    ...
    try {
      ...
      // Cambio 36: el menú de canción sube al nivel Compose; no duplicar cabecera en Estructura.
      ...
      err.innerHTML = "<h4>Cambio número 35 · Error visible en Estructura</h4><p>No se pudo construir el panel. Revisa la primera línea roja de consola.</p>";
  ...
  function scheduleRightPanelChart(ctx, root) {
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        status.textContent = "Cambio número 35 · Cargando Chart… intento " + (attempt + 1) + ".";
        if (attempt === 0) ensureChartScriptLoaded();
        if (attempt < 12) {
          setTimeout(() => mount(attempt + 1), 160 + attempt * 120);
        } else {
          status.textContent = "Cambio número 35 · No encontré Studio936SuiteProChart.mountInRightPanel(). Revisa que index.html cargue suite-pro-chart-v235-cambio35.js.";
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 35 · Chart montado. Panel izquierdo = consola por sección limpia.";
