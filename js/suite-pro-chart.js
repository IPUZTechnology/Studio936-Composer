...
/* ── NUEVAS ANIMACIONES PARA PLAYBACK ── */
...
@keyframes beatPulse {
  0% { transform: scale(1); }
  50% { transform: scale(1.08); background: rgba(0,255,204,0.25); }
  100% { transform: scale(1); }
}

.s936-ch-bar.playing {
  animation: barGlow 0.8s ease-in-out;
  background: rgba(0,255,204,0.08) !important;
}

@keyframes barGlow {
  0% { box-shadow: 0 0 0px rgba(0,255,204,0); }
  50% { box-shadow: 0 0 30px rgba(0,255,204,0.15); }
...
.s936-ch-pop .suggestion-btn.sel {
...
.s936-ch-dblbar{height:3px;margin-top:2px;background:linear-gradient(to right,rgba(255,255,255,.15) 0,rgba(255,255,255,.15) calc(100% - 4px),rgba(255,255,255,.5) calc(100% - 4px),rgba(255,255,255,.5) 100%)}
  ...
  // ─── POPUP MEJORADO CON SUGERENCIAS ──────────────────────────────────────
  ...
  function showBeatPop(targetEl, label, currentVal, onSave) {
    ...
    // SUGERENCIAS DE ACORDES COMUNES
  ...
  // ─── RENDER BEAT (MEJORADO - SIN VOICING EN BEATS VACÍOS) ──────────────
  function renderBeat(sectionKey, barIndex, beatIndex, beatVal, repeatRef, inst, voicingLibrary, onRerender) {
    ...
    if (parsed) {
      ...
      // ★ MEJORA: Solo muestra % sin voicing
    ...
    // ★ MEJORA: Solo agregar voicing si el beat tiene acorde propio
    ...
    if (parsed) {
      ...
      // Si es % NO mostramos voicing (dejamos vacío)
      voicingChordName = null;
    }
  ...
  // ─── RENDER COMPÁS (MEJORADO - SÍMBOLOS DE REPETICIÓN) ─────────────────
  function renderBar({ barIndex, isFirst, sectionKey, beatsData, barInfo, inst, voicingLibrary, onRerender, prevChord }) {
    ...
    // ★ MEJORA: Símbolo de repetición si el acorde se repite
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    // ★ MEJORA: Botón Play/Pause en el header
    ...
    arrangement.forEach(item => {
      ...
      // ★ MEJORA: Símbolo de fin de repetición si la sección tiene repeat
      if (item.repeat) {
  ...
  // ─── SISTEMA DE PLAYBACK (NUEVO) ─────────────────────────────────────────
  function startPlayback(bpm = 120) {
    if (_isPlaying) return;
    _isPlaying = true;
    _currentBeatIndex = 0;
    
    // Obtener todos los beats del chart
    const beats = document.querySelectorAll('.s936-ch-beat');
    if (!beats.length) return;
    
    const beatDuration = 60000 / bpm / 4; // Duración de cada beat en ms
    ...
    function highlightNextBeat() {
      ...
      if (beat) {
        ...
        // Resaltar también el compás padre
