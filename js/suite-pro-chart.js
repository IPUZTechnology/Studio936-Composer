// Studio 936 Composer - Chart View v1.6.0 (MEJORADO)
// iReal Book style: 4 compases × 4 tiempos + voicings + selector instrumento
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v1.6.0";
  const STYLE_ID = "s936-chart-v160";
...
.s936-ch-bar{border-right:1px solid rgba(255,255,255,.12);padding:2px 2px 4px 2px;position:relative;box-sizing:border-box;transition:background .1s;min-height:140px}
...
.s936-ch-bar-head{padding:2px 4px 4px;min-height:20px;display:flex;align-items:center;justify-content:space-between}
.s936-ch-beats{display:grid;grid-template-columns:repeat(4,1fr);gap:3px;padding:0 2px}
.s936-ch-beat{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:4px;display:flex;flex-direction:column;padding:4px 3px 3px;cursor:pointer;transition:background .1s,border-color .1s;position:relative;min-width:0;min-height:120px}
...
.s936-ch-beat.playing{animation:beatPulse 0.5s ease-in-out;border-color:#00ffcc!important;box-shadow:0 0 20px rgba(0,255,204,0.3)}
@keyframes beatPulse{0%{transform:scale(1)}50%{transform:scale(1.08);background:rgba(0,255,204,0.25)}100%{transform:scale(1)}}
.s936-ch-bar.playing{animation:barGlow 0.8s ease-in-out;background:rgba(0,255,204,0.08)!important}
@keyframes barGlow{0%{box-shadow:0 0 0px rgba(0,255,204,0)}50%{box-shadow:0 0 30px rgba(0,255,204,0.15)}100%{box-shadow:0 0 0px rgba(0,255,204,0)}}
.s936-ch-beat-num{font-size:.34rem;color:rgba(255,255,255,.28);font-weight:700;line-height:1;margin-bottom:3px;text-align:center}
.s936-ch-beat-chord{display:flex;align-items:center;justify-content:center;gap:1px;min-height:24px;margin-bottom:3px;padding:0 2px}
.s936-ch-beat-root{font-size:.85rem;font-weight:900;color:#fff;line-height:1.2}
.s936-ch-beat-qual{font-size:.48rem;font-weight:700;color:rgba(255,255,255,.6);vertical-align:super;line-height:1}
.s936-ch-beat-bass{font-size:.4rem;color:#ff5bea;font-weight:700}
...
.s936-ch-beat-empty-label{font-size:.6rem;color:rgba(255,255,255,.15);font-weight:400;text-align:center}
.s936-ch-repeat-start{display:inline-block;font-size:.7rem;color:#ffe066;font-weight:900;margin-right:4px}
.s936-ch-repeat-end{display:inline-block;font-size:.7rem;color:#ffe066;font-weight:900;margin-left:4px}
.s936-ch-beat-voicing{flex:1;display:flex;align-items:center;justify-content:center;min-height:40px;margin-top:2px;padding:2px 0}
.s936-ch-piano-mini{height:40px;width:100%;position:relative;border:1px solid rgba(255,255,255,.2);border-radius:3px;overflow:hidden;background:#1e1e1e}
...
.s936-ch-pw.black-k{background:#1a1a1a;z-index:2;top:0;height:60%;border-radius:0 0 2px 2px;border:1px solid #555}
.s936-ch-pw.hit-k{background:#00ffcc!important;box-shadow:0 0 8px rgba(0,255,204,.9)}
.s936-ch-fret-mini{height:40px;width:100%;position:relative;border:1px solid rgba(86,96,106,.5);border-radius:3px;overflow:hidden;background:linear-gradient(90deg,rgba(139,91,49,.4),rgba(70,45,26,.2))}
...
.s936-ch-fd{position:absolute;width:8px;height:8px;border-radius:50%;background:#00ffcc;transform:translate(-50%,-50%);box-shadow:0 0 6px rgba(0,255,204,.7)}
...
.s936-ch-pop{position:absolute;top:0;left:0;right:0;background:linear-gradient(135deg,#0a0d1a,#1a1f35);border:2px solid rgba(0,255,204,0.3);border-radius:16px;z-index:50;padding:16px;box-shadow:0 20px 60px rgba(0,0,0,0.9);min-width:260px;backdrop-filter:blur(10px)}
.s936-ch-pop label{font-size:.42rem;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.5px;display:block;margin-bottom:3px;font-weight:700}
.s936-ch-pop input{width:100%;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.2);border-radius:8px;color:#fff;font-size:.72rem;font-weight:700;padding:6px 8px;outline:none;box-sizing:border-box}
.s936-ch-pop input:focus{border-color:#00ffcc}
.s936-ch-pop-acts{display:flex;gap:3px;margin-top:8px}
.s936-ch-pop-btn{flex:1;border-radius:8px;font-size:.52rem;font-weight:700;padding:6px;cursor:pointer;border:1px solid;text-align:center;transition:all .2s}
.s936-ch-pop-btn.ok{background:rgba(0,255,204,.15);border-color:rgba(0,255,204,.35);color:#bfffee}
.s936-ch-pop-btn.ok:hover{background:rgba(0,255,204,.25)}
.s936-ch-pop-btn.del{flex:0 0 auto;background:rgba(255,80,80,.1);border-color:rgba(255,80,80,.3);color:#ff8080;padding:6px 12px}
.s936-ch-pop-btn.del:hover{background:rgba(255,80,80,.2)}
.s936-ch-pop .preview{background:rgba(255,255,255,0.05);padding:12px;border-radius:10px;font-size:1.3rem;text-align:center;border:1px solid rgba(255,255,255,0.08);margin:8px 0;font-family:'Jazz','Times New Roman',serif;color:#fff;min-height:32px;display:flex;align-items:center;justify-content:center}
.s936-ch-pop .preview.has-chord{border-color:rgba(0,255,204,0.3);color:#00ffcc}
.s936-ch-pop .suggestions{display:grid;grid-template-columns:repeat(4,1fr);gap:4px;margin:8px 0}
.s936-ch-pop .suggestion-btn{background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:6px;padding:5px 4px;color:rgba(255,255,255,0.7);font-size:.58rem;font-weight:600;cursor:pointer;transition:all .2s;text-align:center}
.s936-ch-pop .suggestion-btn:hover{background:rgba(0,255,204,0.15);border-color:rgba(0,255,204,0.3);color:#00ffcc}
.s936-ch-pop .suggestion-btn.sel{background:rgba(0,255,204,0.2);border-color:#00ffcc;color:#00ffcc}
.s936-ch-dblbar{height:3px;margin-top:2px;background:linear-gradient(to right,rgba(255,255,255,.15) 0,rgba(255,255,255,.15) calc(100% - 4px),rgba(255,255,255,.5) calc(100% - 4px),rgba(255,255,255,.5) 100%)}
  ...
  function calcFretVoicing(chordName, inst) {
    ...
    let cleanName = String(chordName).toUpperCase().trim().replace(/\s+/g, "")
      .replace(/MINOR/g, "m").replace(/MAJOR/g, "maj").replace(/MAJ/g, "maj")
      .replace(/DIM/g, "dim").replace(/AUG/g, "aug").replace(/SUS/g, "sus");
    ...
    if (inst === "guitar") {
      ...
      if (!shape) { const base = cleanName.replace(/[79]/, ""); shape = GUITAR_SHAPES[base]; }
      if (!shape) { shape = GUITAR_SHAPES[String(chordName).toUpperCase().trim().replace(/\s+/g, "")]; }
      return shape ? { frets: shape } : null;
    }
    if (inst === "ukulele") {
      let shape = UKU_SHAPES[cleanName];
      if (!shape) { const base = cleanName.replace(/[79]/, ""); shape = UKU_SHAPES[base]; }
      if (!shape) { shape = UKU_SHAPES[String(chordName).toUpperCase().trim().replace(/\s+/g, "")]; }
      return shape ? { frets: shape } : null;
    }
    if (inst === "bass") {
  ...
  function showBeatPop(targetEl, label, currentVal, onSave) {
    ...
    const ACCS  = ["♮","#","b"];
    const QUALS = [
      ["", "Mayor"], ["m", "Menor"], ["7", "Dom 7"], ["m7", "m7"],
      ["maj7", "Maj7"], ["m7b5", "m7b5"], ["dim", "Dim"], ["dim7", "Dim7"],
      ["aug", "Aug"], ["sus4", "Sus4"], ["sus2", "Sus2"],
      ["9", "9"], ["m9", "m9"], ["maj9", "Maj9"],
      ["11", "11"], ["13", "13"], ["add9", "add9"],
      ["6", "6"], ["m6", "m6"], ["5", "5 (power)"]
    ];

    const SUGGESTIONS = ["C", "D", "E", "F", "G", "A", "B", "Cm", "Dm", "Em", "Fm", "Gm", "Am", "Bm", "C7", "D7", "E7", "G7", "A7", "Cmaj7", "Gmaj7", "Am7", "Dm7"];
    ...
    const popW = 280;
    const left = Math.min(Math.max(8, rect.left - 20), window.innerWidth - popW - 8);
    const top  = rect.bottom + 4 < window.innerHeight - 280 ? rect.bottom + 4 : rect.top - 300;
    pop.style.cssText = [
      "position:fixed",
      "left:" + left + "px",
      "top:" + top + "px",
    ...
    function refreshPreview() {
      ...
      preview.className = "preview" + (name ? " has-chord" : "");
    }

    const sugLbl = document.createElement("div");
    sugLbl.className = "s936-picker-label";
    sugLbl.textContent = "Sugerencias";
    pop.appendChild(sugLbl);

    const sugGrid = document.createElement("div");
    sugGrid.className = "suggestions";
    ...
    rootGrid.style.cssText = "display:grid;grid-template-columns:repeat(7,1fr);gap:2px;margin-bottom:4px";
    const rootBtns = {};
    ROOTS.forEach(r => {
      const btn = document.createElement("button");
      btn.className = "s936-picker-btn" + (r === selRoot ? " sel" : "");
      btn.textContent = r;
      btn.style.cssText = "background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.14);border-radius:4px;color:rgba(255,255,255,.75);font-size:.58rem;font-weight:700;padding:4px 2px;cursor:pointer;text-align:center;transition:all .1s";
      btn.onclick = (e) => {
        e.stopPropagation();
        selRoot = r;
    ...
    accRow.style.cssText = "display:flex;gap:3px;margin-bottom:5px";
    const accBtns = {};
    ACCS.forEach(a => {
      const btn = document.createElement("button");
      btn.className = "s936-picker-btn" + (a === selAcc ? " sel" : "");
      btn.textContent = a === "♮" ? "♮" : (a === "#" ? "#" : "b");
      btn.style.cssText = "flex:1;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.14);border-radius:4px;color:rgba(255,255,255,.75);font-size:.52rem;font-weight:700;padding:4px;cursor:pointer;text-align:center;transition:all .1s";
      btn.onclick = (e) => {
        e.stopPropagation();
        selAcc = a;
    ...
    qualGrid.style.cssText = "display:grid;grid-template-columns:repeat(4,1fr);gap:2px;margin-bottom:5px";
    const qualBtns = {};
    QUALS.forEach(([q, lbTxt]) => {
      const btn = document.createElement("button");
      btn.className = "s936-picker-btn" + (q === selQual ? " sel" : "");
      btn.textContent = lbTxt;
      btn.style.cssText = "background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.14);border-radius:4px;color:rgba(255,255,255,.75);font-size:.44rem;font-weight:600;padding:4px 2px;cursor:pointer;text-align:center;transition:all .1s";
    ...
    okBtn.className = "s936-ch-pop-btn ok";
    okBtn.textContent = "✓ Aplicar";
    const delBtn = document.createElement("button");
    delBtn.className = "s936-ch-pop-btn del";
  ...
  function renderBar({ barIndex, isFirst, sectionKey, beatsData, barInfo, inst, voicingLibrary, onRerender, prevChord }) {
    ...
    const repeatStart = document.createElement("span");
    repeatStart.className = "s936-ch-repeat-start";
    if (barIndex > 0 && barInfo?.chord?.name === prevChord) {
      repeatStart.textContent = "𝄆";
    } else {
      repeatStart.textContent = "";
    }

    head.append(num, repeatStart);
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    playBtn.textContent = "▶";
    playBtn.style.cssText = "background:rgba(0,255,204,.15);border:1px solid rgba(0,255,204,.3);border-radius:8px;color:#00ffcc;padding:3px 10px;font-size:.7rem;cursor:pointer;font-weight:700;transition:all .2s";
    playBtn.onmouseover = () => playBtn.style.background = "rgba(0,255,204,.25)";
    playBtn.onmouseout = () => playBtn.style.background = "rgba(0,255,204,.15)";
    playBtn.onclick = (e) => {
      e.stopPropagation();
      if (_isPlaying) {
        stopPlayback();
    ...
    arrangement.forEach(item => {
      ...
      sec.dataset.section = item.section;
      ...
      for (let i = 0; i < totalMeasures; i += COLS) {
        ...
        for (let j = 0; j < COLS; j++) {
          ...
          const barEl = renderBar({
            ...
            prevChord: prevChordName
          });
          line.appendChild(barEl);
          
          if (barInfo?.chord?.name) {
            prevChordName = barInfo.chord.name;
          }
        }
      ...
      if (item.repeat) {
        ...
        repeatEnd.style.cssText = "text-align:right;padding:4px 8px;color:#ffe066;font-size:.7rem;font-weight:900;letter-spacing:1px";
        repeatEnd.textContent = "𝄇 :||";
        sec.appendChild(repeatEnd);
      }
  ...
  function startPlayback(bpm = 120) {
    if (_isPlaying) return;
    _isPlaying = true;
    _currentBeatIndex = 0;
    
    const beats = document.querySelectorAll('.s936-ch-beat');
    if (!beats.length) return;
    
    const beatDuration = 60000 / bpm / 4;
    
    document.querySelectorAll('.s936-ch-beat.playing').forEach(el => el.classList.remove('playing'));
    document.querySelectorAll('.s936-ch-bar.playing').forEach(el => el.classList.remove('playing'));
    
    function highlightNextBeat() {
      if (!_isPlaying) return;
      
      document.querySelectorAll('.s936-ch-beat.playing').forEach(el => el.classList.remove('playing'));
      document.querySelectorAll('.s936-ch-bar.playing').forEach(el => el.classList.remove('playing'));
      
      const beat = beats[_currentBeatIndex];
      if (beat) {
        beat.classList.add('playing');
        const bar = beat.closest('.s936-ch-bar');
        if (bar) bar.classList.add('playing');
        _beatCallbacks.forEach(cb => cb(beat, _currentBeatIndex));
      }
      
      _currentBeatIndex = (_currentBeatIndex + 1) % beats.length;
      _playbackInterval = setTimeout(highlightNextBeat, beatDuration);
    }
  ...
  function stopPlayback() {
    ...
    if (_playbackInterval) {
      clearTimeout(_playbackInterval);
