// Studio 936 Composer - Chart View v2.0.7 Cambio 8 (POPUP CENTRADO + AUDIO INTERNO)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.0.7-cambio-8";
  const STYLE_ID = "s936-chart-v207-cambio8";
  ...
  function startPopupRhythm(midis, bpm) {
    const notes = normalizePopupMidis(midis);
    if (!notes.length) return;
    stopChartPopupAudio();
...
.s936-ch-dblbar{height:3px;margin-top:2px;background:linear-gradient(to right,rgba(255,255,255,.15) 0,rgba(255,255,255,.15) calc(100% - 4px),rgba(255,255,255,.5) calc(100% - 4px),rgba(255,255,255,.5) 100%)}
  ...
  function showBeatPop(targetEl, label, currentVal, inst, onSave, onOpenVoicing) {
    ...
    pop.className = "s936-ch-pop s936-ch-pop-v7 s936-ch-pop-v8";
    ...
    drag.innerHTML = `<span>${label}</span><small>Arrastra · cambio 8</small>`;
    ...
    rhythmBtn.title = "Repetir el acorde al BPM actual";
    ...
    audioRow.append(chordBtn, arpBtn, rhythmBtn, stopBtn);
    leftPane.appendChild(audioRow);

    const audioHint = document.createElement("div");
    audioHint.className = "s936-picker-audio-mini";
    audioHint.textContent = "El audio usa el mapa que estás editando, sin depender de app.js.";
    ...
    rhythmBtn.onclick = (e) => {
      e.stopPropagation();
      startPopupRhythm(currentPopupMidis(), getCurrentChartBpm());
    };
    stopBtn.onclick = (e) => {
      e.stopPropagation();
      stopChartPopupAudio();
    ...
    const doSave = (val, voicing) => { stopChartPopupAudio(); pop.remove(); onSave(val, voicing); };
  ...
  function renderBeat(sectionKey, barIndex, beatIndex, beatVal, inst, voicingLibrary, onRerender) {
    ...
    cell.addEventListener("dblclick", (e) => {
      e.stopPropagation();
      // Cambio 8: no abrir tercera ventana; audio y mapa editable viven en este mismo popup.
    });
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 8 · Popup centrado + audio interno del mapa";
