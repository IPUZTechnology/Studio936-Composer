// Studio 936 Composer - Suite Pro Structure / ADN Module v4.7.0 Cambio 66
...
// Cambio 66: en el pentagrama con varias notas (tresillo/cuatrillo), ahora se
// puede seleccionar y mover CUALQUIERA de las notas ya puestas (antes solo se
// podía volver a mover la nota 1). Clic sobre una nota puesta la selecciona;
// el siguiente clic en el pentagrama la mueve a esa nueva altura.
(function () {
  "use strict";
  ...
  const MODULE_CAMBIO_LABEL = "Cambio 66";

  const STYLE_ID = "s936SuiteProStructureStylesV470Cambio66";
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    for (let i = 0; i < bars; i++) {
      ...
      for (let b = 0; b < 4; b++) {
        ...
        // (ej. "1T Sol4"), en vez del genérico "letra". Se actualiza al elegir altura/duración.
        function updateGuidePlaceholder() {
          const durIcon = LYRIC_NOTE_TEXT[Number(durationWrap.dataset.value)] || "1T";
          const arr = JSON.parse(durationWrap.dataset.pitches || "[]").filter(Boolean);
          const pitchTxt = arr.length ? arr.map(noteSolfa).join("-") : "";
