(function(global){
    ...
    function exportMidi(project,helpers){ console.log('MIDI diagnóstico: export modular.'); if(helpers.flashStatus) helpers.flashStatus('MIDI diagnóstico: export modular.'); const bytes = buildMidiBytes(project, helpers); if(!bytes || !bytes.length){ if(helpers.flashStatus) helpers.flashStatus('Error al exportar MIDI: datos vacíos.'); return; } if(helpers && typeof helpers.download==='function'){ helpers.download(helpers.slug(project.title)+'-arrangement.mid', bytes, 'audio/midi'); } else { const blob = new Blob([bytes],{type:'audio/midi'}); const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=helpers.slug(project.title)+'-arrangement.mid'; a.style.display='none'; document.body.appendChild(a); a.click(); document.body.removeChild(a); setTimeout(()=>URL.revokeObjectURL(url),1000); } helpers.flashStatus('MIDI exportado: acordes, bajo, groove y melodías/solos por sección.'); }
