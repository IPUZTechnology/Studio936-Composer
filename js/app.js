...
*/
function buildSuiteProContent(suite){ const buttons=[['library','library'],['templates','templates'],['transpose','transpose'],['scales','scales'],['chordAI','chordAI'],['drums','drums'],['mixer','mixer'],['record','record'],['midiIn','midiIn'],['pdf','pdf'],['lead','lead'],['practice','practice'],['share','share'],['inspire','inspire'],['theory','theory']]; let close=suite.querySelector('#b25SuiteClose,#v25uxSuiteClose,#v18SuiteClose,.b25SuiteClose,.v25ux-suite-close'); if(!close){ close=document.createElement('button'); close.id='b25SuiteClose'; close.type='button'; close.className='b25SuiteClose v25ux-suite-close'; suite.appendChild(close); } else { if(!close.id) close.id='b25SuiteClose'; close.classList.add('b25SuiteClose','v25ux-suite-close'); } close.type='button'; close.textContent='×'; if(!close.dataset.v18Bound){ close.dataset.v18Bound='1'; close.addEventListener('click',()=>{ const panel=$('v18Suite'); if(panel) panel.classList.remove('v19-open'); const t=$('v19ToolsToggle'); if(t) t.classList.remove('open'); qa('#v25UxBar .v25ux-btn').forEach(x=>x.classList.remove('active')); }); } let inner=suite.querySelector('.v18-suite-inner'); if(!inner){ inner=document.createElement('div'); inner.className='v18-suite-inner'; suite.appendChild(inner); } inner.innerHTML=`<div class="v18-suite-title">${T('suite')}</div><div class="v18-suite-buttons">${buttons.map(([id,label])=>`<button class="v18-pill" id="v18_${id}" type="button">${T(label)}</button>`).join('')}</div><div class="v18-detect"><span id="v18DetectOut">${T('noChord')}</span><button class="v18-mini" id="v18ApplyDetected">${T('applyDetected')}</button><button class="v18-mini" id="v18DrumBtn">${T('drumsOff')}</button></div>`; return suite; }
...
(function(){
  ...
  window.Studio936DebugSuite=function(){
    ...
    return {suiteExists:!!suite,suiteClass:suite?suite.className:null,buttonExists:!!btn,hasClose:!!suite?.querySelector('#v25uxSuiteClose,#v18SuiteClose'),hasButtons:!!suite?.querySelector('.v18-suite-buttons'),buttonCount:suite?suite.querySelectorAll('.v18-pill').length:0,display:cs?cs.display:null,visibility:cs?cs.visibility:null,text:suite?suite.textContent.slice(0,200):'',bodyClass:document.body.className};
  };
