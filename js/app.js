...
// v18 Pro Suite legacy block removed.
// Suite Pro now lives in js/suite-pro.js and owns #s936SuitePro.
// app.js must not create, populate, or style Suite Pro DOM.

/* ---- Script block separator ---- */

// v19 helper kept: automatic guitar/ukulele fretboard + Flow 8 device helper.
// Removed: legacy tools drawer injection.
(() => {
  'use strict';
  const STORAGE_KEY = 'studio936ComposerV25SongStructure';
  const LANG_KEY = 'pianoComposerUiLangV15';
  ...
  function init(){ ensureAutoFret(); ensureFlowPanel(); patchHelp(); refreshText(); }
...
// v25 workspace shell. Suite Pro is delegated to js/suite-pro.js only.
(function(){
  ...
  function makeBar(){
    ...
    bar.addEventListener('click',ev=>{
      ...
      if(window.Studio936SuitePro && typeof window.Studio936SuitePro.close === 'function') window.Studio936SuitePro.close();
      openPanel(p);
    });
  ...
  function closeEditor(){
    ...
    qa('#v25UxBar .v25ux-btn').forEach(b=>{ if(b.dataset.uxOpen !== 'suite') b.classList.remove('active'); });
    if(!(window.Studio936SuitePro && typeof window.Studio936SuitePro.isOpen === 'function' && window.Studio936SuitePro.isOpen())) document.body.classList.remove('v25ux-panel-open');
  }
  function toggleSuite(){
    ...
    console.warn('Studio936SuitePro module is not loaded.');
  }
  function closeAll(){
    closeEditor();
    if(window.Studio936SuitePro && typeof window.Studio936SuitePro.close === 'function') window.Studio936SuitePro.close();
    qa('#v25UxBar .v25ux-btn').forEach(b=>b.classList.remove('active'));
    document.body.classList.remove('v25ux-panel-open');
  }
  ...
  function refresh(){document.body.classList.add('v25ux-clean'); const small=q('.brand small'); if(small) small.textContent='STUDIO 936 COMPOSER v25 UX · MODULAR WORKSPACE'; makeBar(); classifyCards(); updateLabels(); touchSafe();}
...
// v25.1 workspace helper: panel close + arrangement selector. Suite Pro close is owned by suite-pro.js.
(function(){
  const $=id=>document.getElementById(id), q=(s,r=document)=>r.querySelector(s), qa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  ...
  function refreshLabels(){const pc=$('v25uxPanelClose'); if(pc) pc.textContent=L('Cerrar','Close'); const lab=q('#v25uxPartWrap label'); if(lab) lab.textContent=L('Orden actual','Current order'); const title=q('#v25UxBar .ux-title'); if(title) title.textContent=L('Workspace de composición','Composition workspace');}
  ...
  function init(){addPanelClose(); addArrangementSelect(); refreshLabels(); patchStructureButtonNames(); bindRefreshHooks(); const small=q('.brand small'); if(small) small.textContent='STUDIO 936 COMPOSER v25.1 UX · MODULAR WORKSPACE';}
...
// v25.2 panel state sync. Suite Pro state is read from the new module only.
(function(){
  const q=(s,r=document)=>r.querySelector(s), qa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  function suiteIsOpen(){
    return !!(window.Studio936SuitePro && typeof window.Studio936SuitePro.isOpen === 'function' && window.Studio936SuitePro.isOpen());
  }
  function syncPanelState(){
    const ed=q('.editor');
    const open=!!(ed&&ed.classList.contains('ux-open')) || suiteIsOpen();
  ...
  function init(){
    ...
    if(small) small.textContent='STUDIO 936 COMPOSER v25.2 UX · MODULAR PANELS';
...
(function(){
  ...
  function refreshV25_5(){const small=q('.brand small'); if(small) small.textContent='STUDIO 936 COMPOSER v25.5 UX · HEADER CLOSE FIX'; ensureEditorHeader();}
