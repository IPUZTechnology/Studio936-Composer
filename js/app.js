...
(function(){
  ...
  function toggleSuite(){closeEditor(); const suiteApi=window.Studio936SuitePro; const mount=(suiteApi&&typeof suiteApi.ensureSuiteProMounted==='function')?suiteApi.ensureSuiteProMounted:ensureSuiteProMounted; const suite=ensureSuitePanelIntegrity(mount()); if(!suite){if(typeof flashStatus==='function') flashStatus('Suite Pro no está disponible todavía.'); return;} suite.classList.toggle('v19-open'); const open=suite.classList.contains('v19-open'), tb=$('v19ToolsToggle'); if(tb){tb.classList.toggle('open',open); tb.textContent=open?label('close'):label('suite');} qa('#v25UxBar .v25ux-btn').forEach(b=>b.classList.toggle('active',b.dataset.uxOpen==='suite'&&open));}
