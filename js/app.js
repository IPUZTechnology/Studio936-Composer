...
function midiNoteEvents(track,tick,midi,dur,vel=84,ch=0){ midi = clamp(Math.round(midi),0,127); dur = Math.max(12,Math.round(dur)); vel=clamp(Math.round(vel),1,127); track.push({tick,order:2,data:[0x90+ch,midi,vel]}); track.push({tick:tick+dur,order:1,data:[0x80+ch,midi,0]}); }
function swingTicksForStep(step,style){ const st = styles[style] || styles.funk; return st.swing && [2,6,10,14].includes(step%16) ? Math.round((480/4)*st.swing) : 0; }
function addMidiChord(track,tick,notes,dur,vel,ch=0){ notes.forEach((m,i)=>midiNoteEvents(track,tick+i*7,m,dur,vel,ch)); }
...
function buildMidiBytes(){
    ...
    const meta=[metaTempo(project.bpm,0), metaTimeSig(0), metaText(0x03, project.title || 'Song',0), metaText(0x01, `Author: ${project.author || ''} | Style: ${project.style} | A4=${masterA()} Hz`,0)];
    const chords=[metaText(0x03,'Chords / Harmony',0)]; const bass=[metaText(0x03,'Bass',0)]; const melody=[metaText(0x03,'Melody / Solo',0)];
    let tick=0; const parts = arrangementParts();
    parts.forEach(part=>{ const section=part.section; meta.push(metaText(0x06, part.label || sectionNames[section] || section, tick)); const seq = project.sections[section] || []; const sectionSolo = parseSolo(getSectionSolo(section).phrase || ''); let sectionStep=0;
        seq.forEach(item=>{ const st = styles[project.style] || styles.funk; const b = noteToMidi(item.bass) ?? 36; const chNotes = parseNotes(item.notes); const notes = chNotes.length ? chNotes : [60,64,67]; const steps = Math.max(1,Number(item.bars)||1)*16; meta.push(metaText(0x01, `${section}: ${item.name}`, tick + sectionStep*STEP));
            for(let ss=0;ss<steps;ss++){ const stepBar=ss%16; const t = tick + (sectionStep+ss)*STEP + swingTicksForStep(stepBar,project.style); if(st.bass.includes(stepBar)) midiNoteEvents(bass,t,bassPatternNote(b,notes,stepBar,project.style),Math.round(STEP*1.65),82,1); if(st.arp){ const arpSteps = project.style==='ballad' ? [0,2,4,6,8,10,12,14] : [0,3,6,8,11,14]; if(arpSteps.includes(stepBar)) midiNoteEvents(chords,t,notes[(Math.floor(stepBar/2)+Math.floor(ss/16)+1)%notes.length],Math.round(STEP*1.4),66,0); } if(st.chord.includes(stepBar)) addMidiChord(chords,t,notes,Math.round(STEP*1.9),72,0); if(st.ghost.includes(stepBar)) addMidiChord(chords,t,thinChord(notes),Math.round(STEP*.9),38,0); const ev = soloEventStartAt(sectionSolo, sectionStep+ss); if(ev && ev.midi !== null) midiNoteEvents(melody,t+Math.round(STEP*.1),clamp(ev.midi,0,127),Math.round(STEP*ev.dur*.9),86,2); }
            sectionStep += steps;
...
function exportPdf(){ const p=getProject(); download(slug(p.title)+'-lead-sheet.pdf', makePdf(projectLines(p)), 'application/pdf'); flash(T('pdfDone')); }
function buildLeadHtml(p){ return `<div class="v18-lead-sheet"><h1>${esc(p.title)}</h1><h2>${esc(p.author)}</h2><p><b>BPM:</b> ${esc(p.bpm)} · <b>Style:</b> ${esc(p.style)} · <b>A4:</b> ${esc(p.tuningHz||440)} Hz</p>${SONG_ORDER.map(k=>{ const sec=p.sections[k]||[]; if(!sec.length) return ''; return `<section><h3>${esc(sectionName(k))}</h3><div class="v18-chordline">${sec.map(c=>`<span>${esc(c.name)}</span>`).join(' ')}</div>${p.lyrics?.[k]?`<pre>${esc(p.lyrics[k])}</pre>`:''}${p.sectionSolos?.[k]?.phrase?`<p><b>Solo:</b> ${esc(p.sectionSolos[k].phrase)}</p>`:''}</section>`; }).join('')}</div>`; }
function showLeadSheet(){ openModal('lead',T('lead'), buildLeadHtml(getProject())+`<div class="v18-actions"><button class="v18-btn" id="v18PrintLead">Print / PDF</button><button class="v18-btn" id="v18ExportPdf2">${T('pdf')}</button></div>`); $('v18PrintLead').onclick=()=>window.print(); $('v18ExportPdf2').onclick=exportPdf; }
