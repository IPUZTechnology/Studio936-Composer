...
(function(){
  ...
  function toggleSuite(){closeEditor(); if(window.Studio936SuitePro && typeof window.Studio936SuitePro.open==='function'){ window.Studio936SuitePro.open(); const suite=$('v18Suite'); const open=!!suite&&suite.classList.contains('v19-open'); const tb=$('v19ToolsToggle'); if(tb){tb.classList.toggle('open',open); tb.textContent=open?label('close'):label('suite');} qa('#v25UxBar .v25ux-btn').forEach(b=>b.classList.toggle('active',b.dataset.uxOpen==='suite'&&open)); return; } const suite=ensureSuitePanelIntegrity(ensureSuiteProMounted()); if(!suite){if(typeof flashStatus==='function') flashStatus('Suite Pro no está disponible todavía.'); return;} suite.classList.toggle('v19-open'); const open=suite.classList.contains('v19-open'), tb=$('v19ToolsToggle'); if(tb){tb.classList.toggle('open',open); tb.textContent=open?label('close'):label('suite');} qa('#v25UxBar .v25ux-btn').forEach(b=>b.classList.toggle('active',b.dataset.uxOpen==='suite'&&open));}
