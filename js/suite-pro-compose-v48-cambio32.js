// Studio 936 Composer - Suite Pro Compose Coordinator v4.8 Cambio 32
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProComposeStylesCambio32";
...
/* Cambio 32 · Dock limpio: sin banners temporales; menú de canción en nivel Compose */
...
`;
  ...
  function render(ctx) {
    ...
    // Cambio 32: el menú de canción sube al nivel Compose; se quitan banners temporales del dock.
    ...
    try {
      ...
      box.innerHTML = "<b>Cambio número 32 · Estructura no pudo renderizar.</b><br>El error queda visible en pantalla. Revisa la primera línea roja de consola.";
  ...
  function renderComposeSongMenu(ctx, shell) {
    const bar = ctx.el("div", "s936-cmp-songbar");
    const left = ctx.el("div", "s936-cmp-songbar-title", "Consola de composición");
    const right = ctx.el("div", "s936-cmp-songbar-right");

    const btn = ctx.el("button", "s936-cmp-song-menu-btn", "☰ Menú canción");
    btn.type = "button";
    btn.title = "Menú canción";
    const dd = ctx.el("div", "s936-cmp-song-menu-dd");

    const item = (label, fn, cls="") => {
      const b = ctx.el("button", "s936-cmp-song-menu-item " + cls, label);
      b.type = "button";
      b.onclick = (e) => { e.stopPropagation(); dd.classList.remove("open"); try { fn?.(); } catch(err) { console.warn(err); } };
      return b;
    };
    ...
    dd.appendChild(item("Mapa Maestro", () => {
      if (window.Studio936OpenMasterMap) return window.Studio936OpenMasterMap();
      const btns = Array.from(document.querySelectorAll("button,[role='button'],.s936-tab"));
      const target = btns.find(el => /mapa\s+maestro/i.test(String(el.textContent || "")));
    ...
    const wrap = ctx.el("div", "s936-cmp-song-menu-wrap");
    wrap.append(btn, dd);
    ...
    setTimeout(() => {
      try {
        ...
        const master = candidates.find(el => {
          if (el.closest?.(".s936-cmp-songbar")) return false;
