// Studio 936 Composer - Suite Pro Structure / ADN Module v4.4.5 Cambio 40
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV445Cambio40";
  ...
  function register() {
    ...
    window.Studio936SuiteProStructure = { version: "structure-v4.4.5-cambio-40", render };
    window.Studio936SuiteProModules.structure = window.Studio936SuiteProStructure;
  }


  function installDockFlexGuard() {
    if (window.__s936DockFlexGuardCambio40) return;
    window.__s936DockFlexGuardCambio40 = true;
    ...
    const sync = () => {
      try {
        ...
        if (!dock || !chartPanel) return;

        // Dock no debe quedar encima del Chart: si el borde derecho del dock invade el panel,
        // se desplaza el Chart solo lo necesario.
        ...
        const safeGap = 10;
        const overlap = Math.ceil((dockRect.right + safeGap) - panelRect.left);
        if (overlap > 0) {
          chartPanel.style.marginLeft = overlap + "px";
          chartPanel.style.width = "calc(100% - " + overlap + "px)";
          chartPanel.dataset.s936DockFlex = "on";
        } else {
          chartPanel.dataset.s936DockFlex = "off";
        }

        document.documentElement.style.setProperty("--s936-suite-dock-right", Math.ceil(dockRect.right + safeGap) + "px");
      } catch (_) {}
    };
