// Studio 936 Composer - Suite Pro Structure / ADN Module v4.5.6 Cambio 51
...
// Cambio 51: ícono de nota musical (♩ 𝅗𝅥 𝅗𝅥. 𝅝) en vez de <select> de texto para la duración por tiempo en Ly Letra.
(function () {
  "use strict";

  const STYLE_ID = "s936SuiteProStructureStylesV456Cambio51";
...
.s936-lyric-duration-btn{
  ...
  background:rgba(255,224,102,.065);
  color:#ffe066;
  font-size:.92rem;
  line-height:1;
  font-weight:900;
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    for (let i = 0; i < bars; i++) {
      ...
      for (let b = 0; b < 4; b++) {
        ...
        // Cambio 51: ícono de nota en vez de <select> de texto.
        // Clic en el ícono abre un popover con las 4 duraciones como íconos musicales.
        ...
        durationBtn.title = LYRIC_NOTE_LABEL[initialDur];
        durationBtn.setAttribute("aria-label", "Duración: " + LYRIC_NOTE_LABEL[initialDur]);
        ...
        [1, 2, 3, 4].forEach((n) => {
          ...
          opt.onclick = (e) => {
            ...
            durationBtn.title = LYRIC_NOTE_LABEL[n];
            durationPop.querySelectorAll(".s936-lyric-duration-opt").forEach((o) => o.classList.remove("is-active"));
            opt.classList.add("is-active");
            durationPop.hidden = true;
          };
          durationPop.appendChild(opt);
        });

        durationBtn.onclick = (e) => {
          e.stopPropagation();
          const wasHidden = durationPop.hidden;
          panel.querySelectorAll(".s936-lyric-duration-pop").forEach((p) => { p.hidden = true; });
          durationPop.hidden = !wasHidden;
        };

        durationWrap.append(durationBtn, durationPop);
        durationFields.push(durationWrap);

        cell.append(em, ta, durationWrap);
    ...
    function saveLyrics(closeAfterSave) {
      ...
      try {
        window.dispatchEvent(new CustomEvent("studio936:section-lyrics-updated", {
          detail: { section, label, bars, beats: nextBeats, lines: nextLines, durations: nextDurations, source: "structure-lyrics-floating-editor" }
        }));
      } catch(_) {}
