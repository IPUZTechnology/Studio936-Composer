// Studio 936 Composer - Suite Pro Structure / ADN Module v4.3.5 Cambio 30
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV435Cambio30";
  ...
  function register() {
    ...
    window.Studio936SuiteProStructure = { version: "structure-v4.3.5-cambio-30", render };
...
`;
    style.textContent += `
/* Cambio 30 · Dock limpio: datos maestros arriba, menú canción compacto, canales dorados */
...
`;
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 30 · consola por sección limpia + íconos dorados");
    root.appendChild(banner);
  }

  function scheduleRightPanelChart(ctx, root) {
    const status = ctx.el("section", "s936-struct-right-status");
    status.textContent = "Cambio número 30 · Estructura abrió. Chart + consola por sección…";
    root.appendChild(status);

    const ensureChartScriptLoaded = () => {
      const src = "js/suite-pro-chart-v230-cambio30.js";
      const already = Array.from(document.scripts || []).some((script) => String(script.src || "").includes(src));
      if (already) return;
      const script = document.createElement("script");
      script.src = src + "?v=cambio-30-20260629";
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        status.textContent = "Cambio número 30 · Cargando Chart… intento " + (attempt + 1) + ".";
        if (attempt === 0) ensureChartScriptLoaded();
        if (attempt < 12) {
          setTimeout(() => mount(attempt + 1), 160 + attempt * 120);
        } else {
          status.textContent = "Cambio número 30 · No encontré Studio936SuiteProChart.mountInRightPanel(). Revisa que index.html cargue suite-pro-chart-v230-cambio30.js.";
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 30 · Chart montado. Panel izquierdo = consola por sección limpia.";
  ...
  function renderHeader(ctx, root, s, parts) {
    ...
    // Cambio 30: no duplicar título/estilo/BPM en el dock. Se conservan en draft desde el Main.
