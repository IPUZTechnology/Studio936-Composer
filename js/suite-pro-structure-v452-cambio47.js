// Studio 936 Composer - Suite Pro Structure / ADN Module v4.5.2 Cambio 47
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV452Cambio47";
  ...
  function installDockFlexGuard() {
    ...
    const sync = () => {
      try {
        ...
        if (chartPanel) {
          // Cambio 47: no empujar el Chart con margin-left; eso generaba scroll horizontal
...
/* Cambio 47 · Lyric editor inline: no overlay negro, no invade el Chart */
...
/* Cambio 47 · Editor flotante de letra por compás y tiempo */
...
`;
    ...
    style.textContent += `
/* Cambio 47 · Dashboard visual + editor Ly por compás */
#s936SuitePro .s936-zoom-session-console{
...
`;
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    // Cambio 47: editor Ly flotante, movible y redimensionable.
    ...
    panel.setAttribute("aria-label", "Editor flotante de letra por compás y tiempo");

    const head = ctx.el("div", "s936-lyrics-head");
    const title = ctx.el("div", "");
    title.appendChild(ctx.el("b", "", "Ly · Letra por compás / tiempo"));
    title.appendChild(ctx.el("small", "", label + " · mueve esta ventana y escribe mirando el Chart."));
    ...
    const help = ctx.el("div", "s936-lyrics-help", "Cada compás tiene 4 tiempos. Puedes escribir una palabra, sílaba o frase por tiempo. El Chart muestra la frase completa del compás y queda lista para karaoke por beat.");
    ...
    for (let i = 0; i < bars; i++) {
      ...
      lab.appendChild(ctx.el("span", "", "4 tiempos"));
      ...
      for (let b = 0; b < 4; b++) {
        ...
        const em = ctx.el("em", "", "T" + (b + 1));
        const ta = document.createElement("textarea");
        ta.placeholder = "tiempo " + (b + 1);
    ...
    function saveLyrics(closeAfterSave) {
      ...
      try {
        window.dispatchEvent(new CustomEvent("studio936:section-lyrics-updated", {
          detail: { section, label, bars, beats: nextBeats, lines: nextLines, source: "structure-lyrics-floating-editor" }
        }));
      } catch(_) {}
    ...
    // Posición inicial: cerca del borde derecho del dock, sin tapar el header.
    try {
      const dock = document.getElementById("s936SuitePro")?.getBoundingClientRect?.();
      const left = dock ? Math.min(window.innerWidth - 740, Math.max(24, dock.right + 24)) : Math.max(24, window.innerWidth * 0.35);
  ...
  function renderBuilder(ctx, root, s, parts) {
    ...
    if (focusedPart) {
      // Cambio 47: se elimina el header grande "Zoom sección".
  ...
  function partRow(ctx, s, parts, part, index) {
    ...
    // Cambio 47: en Zoom sección, la barra del canal solo deja los controles principales.
