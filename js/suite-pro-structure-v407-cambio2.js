// Studio 936 Composer - Suite Pro Structure / ADN Module v4.0.7 Cambio 2
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV407Cambio2";
  ...
  function register() {
    ...
    window.Studio936SuiteProStructure = { version: "structure-v4.0.6-cambio-1", render };
...
`;
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 2 · Chart v2 preservado + visible dentro de Compose");
  ...
  function renderInlineChart(ctx, root) {
    ...
    try { Chart?.unmountFromRightPanel?.(); } catch(_) {}
    ...
    if (!Chart || typeof Chart.render !== "function") {
      panel.innerHTML = '<p style="color:rgba(255,255,255,.55);padding:18px;text-align:center;font-size:.72rem">Chart View no está registrado. Revisa que el script del chart cargue antes de Estructura.</p>';
      return;
    }
    ...
    try {
      ...
      if (!panel.childElementCount) {
        panel.innerHTML = '<p style="color:rgba(255,255,255,.55);padding:18px;text-align:center;font-size:.72rem">El Chart cargó, pero no recibió arreglo. Crea partes en Estructura o revisa el bridge.</p>';
      }
    } catch (error) {
      console.warn("Suite Pro Structure Chart inline:", error);
      panel.innerHTML = '<p style="color:#ffb5b5;padding:18px;text-align:center;font-size:.72rem">Error al renderizar Chart View. Abre consola y revisa la primera línea roja.</p>';
    }
  }
