// Studio 936 Composer - Suite Pro Structure / ADN Module v4.2.5 Cambio 20
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV425Cambio20";
  ...
  function register() {
    ...
    window.Studio936SuiteProStructure = { version: "structure-v4.2.5-cambio-20", render };
...
/* Cambio 20 · Navegación musical visible antes del arreglo */
...
/* Cambio 20 · el panel izquierdo queda como consola, no como editor pesado de marcas */
.s936-session-console-card{
  border-color:rgba(0,255,204,.32)!important;
  background:linear-gradient(135deg,rgba(0,255,204,.08),rgba(255,255,255,.025))!important;
}
...
`;
  ...
  function render(ctx, shell) {
    ...
    try {
      ...
      // Cambio 20: las marcaciones se editan directamente en el Chart; el panel izquierdo queda para consola.
      renderSessionConsole(ctx, root, s, parts);
      renderBuilder(ctx, root, s, parts);
      scheduleRightPanelChart(ctx, root);
    } catch (error) {
      console.error("Suite Pro Structure Cambio 20 render error:", error);
  ...
  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 20 · Marcas en compás + panel listo para consola");
    ...
    const mount = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.mountInRightPanel !== "function") {
        ...
        if (attempt < 12) {
          ...
          status.textContent = "Cambio número 14 · No encontré Studio936SuiteProChart.mountInRightPanel(). Revisa que index.html cargue suite-pro-chart-v213-cambio14.js.";
      ...
      try {
        ...
        if (result && result.ok) {
          status.textContent = "Cambio número 14 · Chart montado en el panel derecho. Bridge de práctica activar activo.";
  ...
  function renderNavigationPanel(ctx, root, s, parts) {
    ...
    card.appendChild(ctx.el("p", "s936-struct-muted", "Define las señales del mapa profesional: repeticiones, Fine, Segno, Coda y casillas. Cambio 20 las deja visibles arriba y dibujadas claramente en el Chart; la obediencia completa en práctica viene después."));
  ...
  function renderSessionConsole(ctx, root, s, parts) {
    ...
    card.appendChild(ctx.el("h4", "", "Mini consola sesión"));
    card.appendChild(ctx.el("p", "s936-struct-muted", "Cambio 20: este espacio queda reservado para practicar la sección/parte. Las marcaciones se ponen directamente sobre los compases del Chart."));

    const row = ctx.el("div", "s936-session-console-row");
    const play = ctx.el("button", "s936-struct-btn primary", "▶ Practicar Chart");
    const loop = ctx.el("button", "s936-struct-btn", "🔁 Loop sección");
    const pulse = ctx.el("button", "s936-struct-btn", "🥁 Pulso");
    const stop = ctx.el("button", "s936-struct-btn danger", "■ Stop");

    play.onclick = () => {
      const panel = document.getElementById("s936-chart-view-panel");
      const ok = window.Studio936SuiteProChart?.startChartRhythmConsole?.(panel, { withPulse: false });
    ...
    loop.onclick = () => {
      try {
        window.dispatchEvent(new CustomEvent("studio936:chart-loop-current-section", {
          detail: { section: s?.currentSection || parts?.[0]?.section || "" }
        }));
      } catch(_) {}
      alert("Loop de sección queda preparado para el siguiente paso. Por ahora usa Practicar Chart.");
    };
    pulse.onclick = () => {
      const panel = document.getElementById("s936-chart-view-panel");
      window.Studio936SuiteProChart?.startChartRhythmConsole?.(panel, { withPulse: true });
    };
    stop.onclick = () => {
    ...
    const note = ctx.el("div", "s936-session-console-note", "Play superior = canción completa. Mini consola = práctica de Chart/sección. Marcas: clic sobre el encabezado de cada compás.");
    card.appendChild(note);

    const marks = readNavigationMarks();
    const summary = ctx.el("div", "s936-session-mark-summary");
    if (marks.length) {
      summary.appendChild(ctx.el("strong", "", "Marcas activas: "));
      marks.slice(0, 5).forEach((mark) => {
        const pill = ctx.el("span", "s936-struct-nav-pill compact", (mark.partLabel || mark.section) + " · C" + ((Number(mark.bar) || 0) + 1) + " · " + navMarkText(mark));
        summary.appendChild(pill);
      });
      if (marks.length > 5) summary.appendChild(ctx.el("span", "s936-struct-muted", " +" + (marks.length - 5) + " más"));
    } else {
      summary.appendChild(ctx.el("span", "s936-struct-muted", "Sin marcas. Haz clic en el encabezado de un compás del Chart para agregarlas."));
    }
