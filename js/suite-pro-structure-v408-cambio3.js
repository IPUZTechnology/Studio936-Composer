// Studio 936 Composer - Suite Pro Structure / ADN Module v4.0.8 Cambio 3
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV408Cambio3";
  ...
  function register() {
    ...
    window.Studio936SuiteProStructure = { version: "structure-v4.0.8-cambio-3", render };
...
`;
  ...
  function render(ctx, shell) {
    ...
    renderInlineChart(ctx, root);
    shell.appendChild(root);
  }

  function renderChangeBanner(ctx, root) {
    const banner = ctx.el("div", "s936-struct-change-banner", "Cambio número 3 · Estructura fuerza Chart visible y lee borrador");
    root.appendChild(banner);
  }

  function renderInlineChart(ctx, root) {
    renderChangeBanner(ctx, root);

    // Cambio 3: antes de crear el panel inline, desmontar solo cualquier Chart flotante viejo.
    try { window.Studio936SuiteProChart?.unmountFromRightPanel?.(); } catch(_) {}

    const card = ctx.el("section", "s936-struct-card s936-struct-chart-card");
    ...
    const ensureChartScriptLoaded = () => {
      const src = "js/suite-pro-chart-v202-cambio3.js";
      const already = Array.from(document.scripts || []).some((s) => String(s.src || "").includes(src));
      if (already) return;
      const script = document.createElement("script");
      script.src = src + "?v=cambio-3-20260628";
      script.dataset.s936Cambio3ChartLoader = "true";
    ...
    const paintChart = (attempt = 0) => {
      ...
      if (!Chart || typeof Chart.render !== "function") {
        panel.innerHTML = status("Cargando Chart View dentro de Estructura… intento " + (attempt + 1) + ".", "wait");
        if (attempt === 0) ensureChartScriptLoaded();
        if (attempt < 8) {
          setTimeout(() => paintChart(attempt + 1), 140 + attempt * 120);
        } else {
          panel.innerHTML = status("Chart View no está registrado. Revisa que exista js/suite-pro-chart-v202-cambio3.js y que index.html lo cargue antes de Estructura.", "error");
        }
        return;
      }
      ...
      try {
        ...
        console.warn("Suite Pro Structure Chart inline cambio 3:", error);
        panel.innerHTML = status("Error al renderizar Chart View. Abre consola y revisa la primera línea roja.", "error");
      }
    };
