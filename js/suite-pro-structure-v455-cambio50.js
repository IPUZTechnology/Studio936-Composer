// Studio 936 Composer - Suite Pro Structure / ADN Module v4.5.5 Cambio 50
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProStructureStylesV455Cambio50";
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    for (let i = 0; i < bars; i++) {
      ...
      for (let b = 0; b < 4; b++) {
        ...
        const durationRow = ctx.el("div", "s936-lyric-duration-row");
        const durationLabel = ctx.el("span", "", "duración");
        const dur = document.createElement("select");
        dur.className = "s936-lyric-duration";
        dur.title = "Duración de esta palabra/sílaba: 1, 2, 3 o 4 tiempos.";
        dur.dataset.bar = String(i);
        dur.dataset.beat = String(b);
        [1,2,3,4].forEach((n) => {
        ...
        dur.value = String(Math.max(1, Math.min(4, Number(barDurations[String(b)] || barDurations[b] || 1))));
        durationFields.push(dur);
        durationRow.append(durationLabel, dur);

        cell.append(em, ta, durationRow);
    ...
    function saveLyrics(closeAfterSave) {
      ...
      durationFields.forEach((field) => {
        const bar = field.dataset.bar;
        const beat = field.dataset.beat;
        const value = Math.max(1, Math.min(4, Number(field.value) || 1));
