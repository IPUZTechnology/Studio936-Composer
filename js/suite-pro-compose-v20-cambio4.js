// Studio 936 Composer - Suite Pro Compose Coordinator v2.0 Cambio 4
...
(function () {
  ...
  const STYLE_ID = "s936SuiteProComposeStylesCambio4";
  ...
  function register() {
    ...
    window.Studio936SuiteProCompose = { version: "compose-v2.0-cambio-4", render };
...
#s936SuitePro .s936-cmp-change-banner{
  ...
  border-radius:14px;
  background:linear-gradient(90deg,rgba(0,255,204,.18),rgba(255,91,234,.12));
  color:#dffff8;
  padding:9px 12px;
  margin:0 0 10px;
  font-size:.70rem;
...
`;
  ...
  function render(ctx) {
    ...
    const banner = ctx.el("div", "s936-cmp-change-banner", "Cambio número 4 · Compose inserta el contenedor antes de renderizar Estructura");
    ...
    // Cambio 4: el shell entra a pantalla ANTES de renderizar el módulo.
    ...
    try {
      ...
      console.error("Suite Pro Compose Cambio 4 render error:", error);
      const box = ctx.el("section", "s936-cmp-render-error");
      box.innerHTML = "<b>Cambio número 4 · Estructura no pudo renderizar.</b><br>Ahora el error queda visible en pantalla. Revisa la primera línea roja de consola.";
