// Studio 936 Composer - Suite Pro Structure / ADN Module v4.6.8 Cambio 64
...
// Cambio 64: un solo pentagrama compartido para 1-4 notas por tiempo (antes
// había un picker por sílaba). Se elige cuántas notas tiene el tiempo (1/2/3/4)
// y se hace clic esa cantidad de veces sobre el MISMO pentagrama — todas las
// notas quedan marcadas juntas ahí, numeradas. El cuadro de texto de la letra
// (ta) YA NO se oculta ni se reemplaza por cuadritos de sílaba — se escribe
// siempre en un único campo, tal como el resto de tiempos.
(function () {
  "use strict";
  ...
  const MODULE_CAMBIO_LABEL = "Cambio 64";

  const STYLE_ID = "s936SuiteProStructureStylesV468Cambio64";
  ...
  function buildSequentialStaffPicker(ctx, count, initialPitches, onChange) {
    ...
    function renderDots() {
      Object.values(rowByPitch).forEach((row) => {
        ...
        const old = row.querySelector(".s936-note-seq-dot");
        if (old) old.remove();
      });
      let filled = 0;
      ...
      const nextIdx = pitches.findIndex((p) => !p);
      if (count === 1) {
        label.textContent = pitches[0] ? noteSolfa(pitches[0]) : "Sin nota";
      } else if (nextIdx === -1) {
        label.textContent = "Las " + count + " notas puestas — clic para rehacer";
      } else {
        label.textContent = "Nota " + (nextIdx + 1) + " de " + count;
      }
    }
    renderDots();
  ...
  function openSectionLyricsEditor(ctx, s, part, parts) {
    ...
    for (let i = 0; i < bars; i++) {
      ...
      for (let b = 0; b < 4; b++) {
        ...
        subdivideRow.appendChild(ctx.el("span", "s936-lyric-subdivide-label", "Cuántas notas en este tiempo"));
