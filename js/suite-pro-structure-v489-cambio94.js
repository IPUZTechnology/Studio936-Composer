// Studio 936 Composer - Suite Pro Structure / ADN Module v4.8.9 Cambio 94
...
// Cambio 94: el Zoom a una sección ya NO se recuerda entre visitas (antes,
// si la última vez hiciste Zoom a "Intro", la próxima vez que abrías la app
// saltaba directo ahí en vez de mostrar la lista completa de secciones).
// Ahora cada recarga siempre empieza mostrando todas las sesiones; el Zoom
// sigue funcionando igual mientras usas la app, solo que ya no persiste de
// una visita a otra. (El Cambio 93 de limpieza CSS fue revertido por un bug
// visual — este Cambio 94 parte de la base confirmada del Cambio 91.)
(function () {
  "use strict";
  ...
  const MODULE_CAMBIO_LABEL = "Cambio 94";

  const STYLE_ID = "s936SuiteProStructureStylesV489Cambio94";
  ...
  function loadState() {
    try {
      const restored = Object.assign({}, DEFAULT_STATE, JSON.parse(localStorage.getItem(STATE_KEY) || "{}"));
      // Cambio 94: "focusSection" (el Zoom a una sección) YA NO se recupera
      // entre visitas — antes, si la última vez hiciste Zoom a "Intro", la
      // próxima vez que abrías la app saltaba directo ahí, sin mostrar
      // primero la lista completa de secciones. Ahora cada recarga siempre
      // arranca mostrando todas las sesiones; el Zoom sigue funcionando
      // igual mientras usas la app, solo que ya no se "recuerda" para la
      // próxima vez — cada visita, tú decides a cuál sección hacer Zoom.
      restored.focusSection = "";
      return restored;
    }
...
#s936SuitePro .s936-struct-card.s936-struct-arrangement-full{
  ...
  /* Cambio 91 (HOTFIX): esta tarjeta tenía su PROPIO scroll independiente
...
.s936-lyrics-float-panel{
  ...
  /* Cambio 78: el panel ahora llena por defecto desde su posición (top:96px)
...
.s936-lyrics-float-panel{
  /* Cambio 68: se quita el !important del ancho — con !important, ganaba
     ...
     porque no tenía !important). Ahora resize:both funciona en las dos
     direcciones, igual que ya funcionaba el alto. */
  width:min(1180px,calc(100vw - 34px));
  /* Cambio 83: en vez de calcular un alto exacto por JS (que podía medirse
     ...
     mismo calcula la altura real del contenido, sin adivinar. SIN !important
