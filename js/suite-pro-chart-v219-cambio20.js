// Studio 936 Composer - Chart View v2.1.9 Cambio 20 (MARCAS CONTEXTUALES EN COMPÁS + PANEL IZQUIERDO LIBRE)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.1.9-cambio-20";
  const STYLE_ID = "s936-chart-v219-cambio20";
  ...
  function scheduleChartPracticeGroove(step, mode, bridgeState) {
    ...
    const chordMidis = chordToChartMidis(chordName);
  ...
  function startChartCentralPracticeBridge({ withPulse = false } = {}) {
    ...
    try {
      window.dispatchEvent(new CustomEvent("studio936:chart-practice-start", {
        detail: { bpm, style, withPulse: !!withPulse, centralStarted: started, version: VERSION }
      }));
    } catch(_) {}
  ...
  function startChartRhythmConsole(container, { withPulse = false } = {}) {
    ...
    if (sourceEl) {
      ...
      sourceEl.textContent = bridgeState.source === "audio-engine"
        ...
        : (isMain ? "Motor: Main/Groove · BPM " + bridgeState.bpm : "Motor: Interno fallback · BPM " + bridgeState.bpm);
...
/* Cambio 20 · escenario Chart sin borrar los comandos Groove del Main */
...
/* Cambio 20 · menú contextual profesional por compás */
.s936-ch-bar-head{
  cursor:pointer;
  user-select:none;
}
...
.s936-ch-nav-badge[data-nav-type="loopEnd"]{
  ...
  function formatNavigationMark(mark) {
    ...
    if (mark.type === "bis" || mark.type === "repeatEnd") {
      ...
      return `${info.symbol} x${repeats}`;
    }
    if (mark.type === "ending1" || mark.type === "ending2") return info.symbol + " " + info.label;
  ...
  function sectionMarks(sectionKey) {
    return readNavigationMarks().filter((mark) => String(mark.section || "") === String(sectionKey || ""));
  }
  ...
  function upsertNavigationMark(sectionKey, barIndex, type, repeats) {
    ...
    marks.push({
      ...
      source: "chart-bar-context-cambio20",
  ...
  function showBarContextMenu(event, { sectionKey, barIndex, onRerender }) {
    ...
    menu.className = "s936-bar-menu";
    menu.onclick = (e) => e.stopPropagation();

    const title = document.createElement("div");
    title.className = "s936-bar-menu-title";
    title.innerHTML = `<span>Compás ${Number(barIndex) + 1}</span><small>${sectionKey}</small>`;
    menu.appendChild(title);

    function group(label) {
      const g = document.createElement("div");
      g.className = "s936-bar-menu-group";
      const l = document.createElement("div");
      l.className = "s936-bar-menu-group-label";
      l.textContent = label;
    ...
    function addButton(grid, label, fn, cls) {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.textContent = label;
      if (cls) btn.className = cls;
      btn.onclick = () => {
        try { fn(); } catch (error) { console.error("Chart bar context action failed:", error); }
        closeBarContextMenus();
        onRerender?.();
      };
    ...
    const rhythmGrid = group("Ritmo rápido del compás");
    addButton(rhythmGrid, "♩ Tocar + sostener", () => applyBarRhythmPreset(sectionKey, barIndex, "hitHold"));
    addButton(rhythmGrid, "𝄽 Silenciar compás", () => applyBarRhythmPreset(sectionKey, barIndex, "restBar"));
    addButton(rhythmGrid, "% Repetir anterior", () => applyBarRhythmPreset(sectionKey, barIndex, "repeatBar"), "warn");

    const repeatGrid = group("Repetición / rango");
    addButton(repeatGrid, "𝄆 Inicio repetición", () => upsertNavigationMark(sectionKey, barIndex, "repeatStart", 2));
    addButton(repeatGrid, "𝄇 Fin x2", () => upsertNavigationMark(sectionKey, barIndex, "repeatEnd", 2), "warn");
    addButton(repeatGrid, "𝄇 Fin x3", () => upsertNavigationMark(sectionKey, barIndex, "repeatEnd", 3), "warn");
    addButton(repeatGrid, "Loop inicio", () => upsertNavigationMark(sectionKey, barIndex, "loopStart", 2));
    addButton(repeatGrid, "Loop final", () => upsertNavigationMark(sectionKey, barIndex, "loopEnd", 2));

    const endingsGrid = group("Finales y puntos");
    addButton(endingsGrid, "Casilla 1", () => upsertNavigationMark(sectionKey, barIndex, "ending1", 2));
    addButton(endingsGrid, "Casilla 2", () => upsertNavigationMark(sectionKey, barIndex, "ending2", 2));
    addButton(endingsGrid, "Fine", () => upsertNavigationMark(sectionKey, barIndex, "fine", 2), "stop");
    addButton(endingsGrid, "𝄋 Segno", () => upsertNavigationMark(sectionKey, barIndex, "segno", 2));
    addButton(endingsGrid, "𝄌 Coda", () => upsertNavigationMark(sectionKey, barIndex, "coda", 2));

    const cleanGrid = group("Limpieza");
    addButton(cleanGrid, "Quitar marcas del compás", () => clearNavigationMarksForBar(sectionKey, barIndex), "danger");

    document.body.append(overlay, menu);
    const x = Math.min(Math.max(8, event.clientX || 8), window.innerWidth - 270);
    const y = Math.min(Math.max(8, event.clientY || 8), window.innerHeight - 360);
  ...
  function renderBar({ barIndex, sectionKey, beatsData, rhythmData, barInfo, inst, voicingLibrary, onRerender, navMarks }) {
    ...
    head.title = "Cambio 20: clic para marcar ritmo/repetición de este compás.";
    head.tabIndex = 0;
  ...
  function render({ container, instrument, onChordEdit } = {}) {
    ...
    changeBanner.textContent = "Cambio número 20 · Marcas directas en compás + panel izquierdo libre";
    ...
    // Cambio 14: mini consola ligera para practicar el Chart al BPM e iluminar cada celda.
    ...
    consoleTitle.textContent = "🎛 Mini consola práctica · motor musical";
    const playChartBtn = document.createElement("button");
    playChartBtn.className = "s936-ch-console-btn";
    playChartBtn.textContent = "▶ Practicar Chart";
    playChartBtn.title = "Practica el Chart al BPM, ilumina cada celda y mantiene el acorde durante el compás.";
    ...
    const consoleSource = document.createElement("span");
    consoleSource.className = "s936-ch-console-source";
    consoleSource.textContent = "Motor: listo";
    ...
    playChartBtn.onclick = (e) => {
      ...
      if (ok) {
        ...
        consoleStatus.textContent = "Practicando";
    ...
    miniConsole.append(consoleTitle, playChartBtn, pulseChartBtn, stopChartBtn, consoleStatus, consoleSource);
      ...
      if (secMarks.length) {
        ...
        secMarks
          ...
          .forEach((mark) => {
            ...
            badgeMark.textContent = "C" + ((Number(mark.bar) || 0) + 1) + " · " + formatNavigationMark(mark);
  ...
  function mountInRightPanel({ onChordEdit } = {}) {
    ...
    // Cambio 20: NO escondemos los comandos Groove/Play/Metronomo del Main.
