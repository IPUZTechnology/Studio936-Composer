// Studio 936 Composer - Chart View v2.4.5 Cambio 45 (LYRICS POR COMPÁS)
...
window.Studio936SuiteProChart = (() => {
  "use strict";
  const VERSION = "chart-v2.4.5-cambio-45";
  const STYLE_ID = "s936-chart-v245-cambio45";
...
`;
  ...
  function lyricForBar(sectionKey, barIndex) {
    try {
      ...
      const lines = sec.lines || sec.bars || sec || {};
      return String(lines[String(barIndex)] || lines[barIndex] || "").trim();
    } catch(_) {
      return "";
    }
  }
  ...
  function renderBar({ barIndex, sectionKey, beatsData, rhythmData, barInfo, inst, voicingLibrary, onRerender, navMarks }) {
    ...
    const lyricLine = lyricForBar(sectionKey, barIndex);
    if (lyricLine) {
      const lyricEl = document.createElement("div");
      lyricEl.className = "s936-ch-lyric-line";
      lyricEl.title = lyricLine;
